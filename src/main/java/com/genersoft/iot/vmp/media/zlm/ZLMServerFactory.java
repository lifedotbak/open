...
public class ZLMServerFactory {
    ...
    private final Logger logger = LoggerFactory.getLogger("ZLMServerFactory");
    ...
    public void closeRtpServer(
            MediaServer serverItem, String streamId, CommonCallback<Boolean> callback) {
        if (serverItem == null) {
            callback.run(false);
            return;
        }
        Map<String, Object> param = new HashMap<>();
        param.put("stream_id", streamId);
        zlmresTfulUtils.closeRtpServer(
                serverItem,
                param,
                jsonObject -> {
                    if (jsonObject != null) {
    ...
    public JSONObject startSendRtpPassive(
            ...
            ZLMRESTfulUtils.RequestCallback callback) {
    ...
    public int totalReaderCount(MediaServer mediaServerItem, String app, String streamId) {
        ...
        if (code == 0
                && mediaInfo.getBoolean("online") != null
                && !mediaInfo.getBoolean("online")) {
            logger.warn("查询流({}/{})是否有其它观看者时得到： {}", app, streamId, mediaInfo.getString("msg"));
            return -1;
        }
    ...
    public Boolean updateRtpServerSSRC(MediaServer mediaServerItem, String streamId, String ssrc) {
        ...
        JSONObject jsonObject =
                zlmresTfulUtils.updateRtpServerSSRC(mediaServerItem, streamId, ssrc);
        if (jsonObject == null) {
