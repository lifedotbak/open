...
public class GBStringMsgParser implements MessageParser {
     ...
     * @exception ParseException is thrown when an illegal message has been encountered (and the
    ...
    public SIPMessage parseSIPMessage(
            ...
            if (currentLine.length() == 0) {
                ...
                if (isFirstLine) {
                    ...
                    if (firstChar == '\t' || firstChar == ' ') {
                        ...
                        if (currentHeader != null && message != null) {
                            processHeader(
                                    currentHeader, message, parseExceptionListener, msgBuffer);
                        }
        ...
        if (readBody && message.getContentLength() != null) {
            if (message.getContentLength().getContentLength() != 0) {
                ...
                System.arraycopy(msgBuffer, i, body, 0, bodyLength);
                message.setMessageContent(
                        body, !strict, computeContentLengthFromMessage, bodyLength);
    ...
    protected SIPMessage processFirstLine(
            String firstLine, ParseExceptionListener parseExceptionListener, byte[] msgBuffer)
            throws ParseException {
        SIPMessage message;
     ...
     *     <p>public HostPort parseHostPort(String hostport) throws ParseException { Lexer lexer =
     *     new Lexer("charLexer", hostport); return new HostNameParser(lexer).hostPort();
     *     <p>}
     */
    ...
    protected void processHeader(
        ...
        try {
            ...
            if (parseExceptionListener != null) {
                ...
                try {
                    parseExceptionListener.handleException(
                            ex, message, headerClass, header, new String(rawMessage, "UTF-8"));
                } catch (UnsupportedEncodingException e) {
                    e.printStackTrace();
                }
