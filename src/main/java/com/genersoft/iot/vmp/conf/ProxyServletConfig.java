...
public class ProxyServletConfig {
    ...
    public ServletRegistrationBean zlmServletRegistrationBean() {
        ServletRegistrationBean servletRegistrationBean =
                new ServletRegistrationBean(new ZlmProxyServlet(), "/zlm/*");
    ...
    public ServletRegistrationBean recordServletRegistrationBean() {
        ServletRegistrationBean servletRegistrationBean =
                new ServletRegistrationBean(new RecordProxyServlet(), "/record_proxy/*");
    ...
    class ZlmProxyServlet extends ProxyServlet {
        ...
        protected String getTargetUri(HttpServletRequest servletRequest) {
            ...
            if (mediaInfo != null) {
                ...
                // requestURI.substring(requestURI.indexOf(mediaInfo.getId())+
                // mediaInfo.getId().length());
                uri = String.format("http://%s:%s", mediaInfo.getIp(), mediaInfo.getHttpPort());
            } else {
                uri =
                        "http://127.0.0.1:"
                                + serverPort
                                + "/index/hook/null"; // 只是一个能返回404的请求而已， 其他的也可以
            }
            return uri;
        }
    ...
    class RecordProxyServlet extends ProxyServlet {
        ...
        protected String rewriteQueryStringFromRequest(
            ...
            String remoteHost =
                    String.format(
                            "http://%s:%s",
                            mediaInfo.getStreamIp(), mediaInfo.getRecordAssistPort());
            if (!ObjectUtils.isEmpty(queryStr)) {
        ...
        @Override
        protected void handleRequestException(
                HttpRequest proxyRequest, HttpResponse proxyResponse, Exception e) {
            try {
        ...
        protected String getTargetUri(HttpServletRequest servletRequest) {
            ...
            if (mediaInfo != null) {
                ...
                uri =
                        String.format(
                                "http://%s:%s", mediaInfo.getIp(), mediaInfo.getRecordAssistPort());
            } else {
                uri =
                        "http://127.0.0.1:"
                                + serverPort
