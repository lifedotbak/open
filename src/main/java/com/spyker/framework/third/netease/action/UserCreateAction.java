 ...
 * <p>
 * token：网易云信IM账号的密码。创建 IM accid时可以由开发者app的服务端指定。若未指定，则云信会自动生成一个IM
 * token，并返回给开发者。客户端登录时，需要传参accid与token给云信服务器鉴权。
 * token没有过期的概念，除非人为更改。只有最新的token才是唯一有效的。 当登录时使用非最新的token，将会返回的错误码 302。
 * <p>
 * <p>
