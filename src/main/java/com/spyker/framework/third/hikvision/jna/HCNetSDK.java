...
public interface HCNetSDK extends Library {
    ...
    boolean NET_DVR_SetDVRConfig(
            int lUserID, int dwCommand, int lChannel, Pointer lpInBuffer, int dwInBufferSize);

    boolean NET_DVR_CaptureJPEGPicture(
            int lUserID, int lChannel, NET_DVR_JPEGPARA lpJpegPara, byte[] sPicFileName);
    ...
    int NET_DVR_GetFileByTime_V40(
            int lUserID, String sSavedFileName, NET_DVR_PLAYCOND pDownloadCond);

    boolean NET_DVR_StopPlayBack(int lPlayHandle);

    boolean NET_DVR_PlayBackControl(
            int lPlayHandle, int dwControlCode, int dwInValue, IntByReference LPOutValue);
    ...
    boolean NET_DVR_PTZPreset_Other(
            int lUserID, int lChannel, int dwPTZPresetCmd, int dwPresetIndex);
    ...
    boolean NET_DVR_SetPlayDataCallBack_V40(
            int lPlayHandle, FPlayDataCallBack fPlayDataCallBack, Pointer dwUser);
    ...
    class NET_DVR_IPPARACFG_V40 extends Structure {
        /* IP接入配置结构V40 */
        ...
        public byte[] byAnalogChanEnable =
                new byte[MAX_CHANNUM_V30]; // 模拟通道资源是否启用，从低到高表示1-64通道：0-禁用，1-启用。
        public NET_DVR_IPDEVINFO_V31[] struIPDevInfo =
                new NET_DVR_IPDEVINFO_V31[MAX_IP_DEVICE_V40]; // IP设备信息，下标0对应设备IP
        // ID为1
        public NET_DVR_STREAM_MODE[] struStreamMode =
                new NET_DVR_STREAM_MODE[MAX_CHANNUM_V30]; // 取流模式
    ...
    class NET_DVR_IPCHANINFO_V40 extends Structure {
        ...
        public int
                dwChannel; // IP设备的通道号，例如设备A（HDVR或者NVR设备）的IP通道01，对应的是设备B（DVS）里的通道04，则byChannel=4，如果前端接的是IPC
        // 则byChannel=1。
    ...
    class NET_DVR_DEVICEINFO_V40 extends Structure {
        ...
        public byte
                byCharEncodeType; // 字符编码类型：0- 无字符编码信息(老设备)，1- GB2312(简体中文)，2- GBK，3- BIG5(繁体中文)，4-
    ...
    class NET_DVR_FILECOND_V40 extends Structure {
        ...
        public int
                dwUseCardNo; // 是否带ATM信息进行查询：0-不带ATM信息，1-按交易卡号查询，2-按交易类型查询，3-按交易金额查询，4-按卡号、交易类型及交易金额的组合查询
        ...
        public NET_DVR_SPECIAL_FINDINFO_UNION uSpecialFindInfo =
                new NET_DVR_SPECIAL_FINDINFO_UNION(); // 专有查询条件
    ...
    class NET_DVR_DEVICEINFO_V30 extends Structure {
        ...
        public byte
                byMultiStreamProto; // 是否支持多码流,按位表示,0-不支持,1-支持,bit1-码流3,bit2-码流4,bit7-主码流，bit-8子码流
