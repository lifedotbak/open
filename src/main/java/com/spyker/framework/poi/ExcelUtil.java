...
public class ExcelUtil<T> {
     ...
     * @return 结果
     */
    public AjaxResult exportExcel2File(String fileDirectory, List<T> list, String sheetName) {
        return exportExcel2File(fileDirectory, list, sheetName, StringUtils.EMPTY);
    }
     ...
     * @return 结果
     */
    public AjaxResult exportExcel2File(
            String fileDirectory, List<T> list, String sheetName, String title) {
        this.init(fileDirectory, list, sheetName, title, Excel.Type.EXPORT);
        return exportExcel2File();
    }
     ...
     * @return 结果
     */
    public void exportExcel2HttpServletResponse(
            HttpServletResponse response, List<T> list, String sheetName) {
        exportExcel2HttpServletResponse(response, list, sheetName, StringUtils.EMPTY);
    }
    ...
    public void exportExcel2HttpServletResponse(
        ...
        this.init(null, list, sheetName, title, Excel.Type.EXPORT);
        exportExcel2HttpServletResponse(response);
    }
     ...
     * @return 结果
     */
    public void exportExcel2HttpServletResponse(HttpServletResponse response) {
        try {
            writeSheet();
    ...
    public void exportExcel2OutputStream(
            OutputStream outputStream, List<T> list, String sheetName) {
        exportExcel2OutputStream(outputStream, list, sheetName, StringUtils.EMPTY);
    }

    public void exportExcel2OutputStream(
            OutputStream outputStream, List<T> list, String sheetName, String title) {

        this.init(null, list, sheetName, title, Excel.Type.EXPORT);
        exportExcel2OutputStream(outputStream);
    }

    public void exportExcel2OutputStream(OutputStream outputStream) {
        try {
            writeSheet();
        ...
        if (StringUtils.isNotEmpty(attr.prompt())
            ...
            if (comboArray.length > 15 || StringUtils.join(comboArray).length() > 255) {
                ...
                setXSSFValidationWithHidden(
                        sheet, comboArray, attr.prompt(), 1, 100, column, column);
            } else {
                // 提示信息或只能选择不能输入的列内容.
    ...
    public void setCellVo(Object value, Excel attr, Cell cell) {
        if (Excel.ColumnType.STRING == attr.cellType()
                || Excel.ColumnType.TEXT == attr.cellType()) {
            ...
            cell.setCellValue(
                    ExStringUtils.isNull(cellValue)
                            ? attr.defaultValue()
                            : cellValue + attr.suffix());
        } else if (Excel.ColumnType.NUMERIC == attr.cellType()) {
            if (ExStringUtils.isNotNull(value)) {
                cell.setCellValue(
    ...
    public AjaxResult exportExcel2File() {
        ...
        try {
            ...
            out = new FileOutputStream(getAbsoluteFile(fileDirectory, filename));
            wb.write(out);
     ...
     * @param filename 文件名称
     */
    public String getAbsoluteFile(String fileDirectory, String filename) {
        ...
        if (rows > 0) {
            ...
            for (int i = titleNum + 1; i <= rows; i++) {
                ...
                for (Map.Entry<Integer, Object[]> entry : fieldsMap.entrySet()) {
                    ...
                    if (ExStringUtils.isNotNull(fieldType)) {
                        ...
                        if (StringUtils.isNotEmpty(attr.readConverterExp())) {
                            ...
                            val = dataFormatHandlerAdapter(val, attr, null);
                                ...
                                && ExStringUtils.isNotEmpty(pictures)) {
                            PictureData image =
                                    pictures.get(row.getRowNum() + "_" + entry.getKey());
                            if (image == null) {
                                val = "";
                            } else {
                                byte[] data = image.getData();
                                val = ExFileUtils.writeImportBytes(data);
                            }
                        }
     ...
     * @return 结果
     */
    public AjaxResult importTemplateExcel2File(String fileDirectory, String sheetName) {
        return importTemplateExcel2File(fileDirectory, sheetName, StringUtils.EMPTY);
    }
     ...
     * @return 结果
     */
    public AjaxResult importTemplateExcel2File(
            String fileDirectory, String sheetName, String title) {
        this.init(fileDirectory, null, sheetName, title, Excel.Type.IMPORT);
        return exportExcel2File();
    }

    public void init(
            String fileDirectory, List<T> list, String sheetName, String title, Excel.Type type) {
        if (list == null) {
     ...
     * @return 结果
     */
    public void importTemplateExcel2Stream(HttpServletResponse response, String sheetName) {
        importTemplateExcel2Stream(response, sheetName, StringUtils.EMPTY);
    }
     ...
     * @return 结果
     */
    public void importTemplateExcel2Stream(
            HttpServletResponse response, String sheetName, String title) {
        response.setContentType(
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setCharacterEncoding("utf-8");
    ...
    private Object getValue(Object o, String name) throws Exception {
        if (ExStringUtils.isNotNull(o) && ExStringUtils.isNotEmpty(name)) {
