...
public class AliPushUtils {
    ...
    public static void main(String[] args) {
        ...
        aliyunPushMessage.setBody("这是一个body");
        aliyunPushMessage.setTargetValue("587A6F47DC3442B9BC3B4B72D88BF820");
    ...
    private void pushIos(AliyunPushMessage aliyunPushMessage) {
        ...
        pushRequest.setPushType(aliyunPushMessage.getPushMessageType().getType());
        pushRequest.setDeviceType("ALL"); // 设备类型 ANDROID iOS ALL.
        // 推送配置
        pushRequest.setTitle(aliyunPushMessage.getTitle()); // 消息的标题
        pushRequest.setBody(aliyunPushMessage.getBody()); // 消息的内容
        // 推送配置: iOS
        pushRequest.setIOSBadge(5); // iOS应用图标右上角角标
        pushRequest.setIOSMusic("default"); // iOS通知声音
        pushRequest.setIOSSubtitle(aliyunPushMessage.getIosSubtitle()); // iOS10通知副标题的内容
        pushRequest.setIOSNotificationCategory(
                aliyunPushMessage.getIosNotificationCategory()); // 指定iOS10通知Category
        ...
        pushRequest.setIOSExtParameters(aliyunPushMessage.getExtParameters()); // 通知的扩展属性(注意
        // :
        // 该参数要以json
        // map的格式传入,否则会解析出错)
        ...
        pushRequest.setAndroidOpenType(aliyunPushMessage.getAndroidOpenType().getType()); // 点击通知后动作
        // "APPLICATION"
        // :
        // 打开应用
        // "ACTIVITY" :
        ...
        // 打开URL "NONE" : 无跳转
        pushRequest.setAndroidOpenUrl(
                aliyunPushMessage.getAndroidOpenUrl()); // Android收到推送后打开对应的url,
        // 仅当AndroidOpenType="URL"有效
        pushRequest.setAndroidActivity(
                aliyunPushMessage.getAndroidActivity()); // 设定通知打开的activity，仅当AndroidOpenType
        // ="Activity"有效
        pushRequest.setAndroidMusic("default"); // Android通知音乐
        pushRequest.setAndroidPopupActivity(
                aliyunPushMessage.getAndroidPopupActivity()); // 设置该参数后启动辅助弹窗功能,
        // 此处指定通知点击后跳转的Activity（辅助弹窗的前提条件：1.
        // 集成第三方辅助通道；2.
        // StoreOffline参数设为true）
        pushRequest.setAndroidPopupTitle(aliyunPushMessage.getAndroidPopupTitle());
        pushRequest.setAndroidPopupBody(aliyunPushMessage.getAndroidPopupBody());
        pushRequest.setAndroidExtParameters(aliyunPushMessage.getExtParameters()); // 设定通知的扩展属性。(注意
        // :
        // 该参数要以
        // json
        // map
