...
public class RefundsResult {
    ...
    /**
     * 退款状态 status string[1, 32] 是
     * 退款到银行发现用户的卡作废或者冻结了，导致原路退款银行卡失败，可前往商户平台-交易中心，手动处理此笔退款。 枚举值： SUCCESS：退款成功
     ...
     * AVAILABLE : 可用余额 UNAVAILABLE : 不可用余额 OPERATION : 运营户 BASIC :
     * 基本账户（含可用余额和不可用余额）
     */
     ...
     * 资金账户 funds_account string[1, 32] 否 退款所使用资金对应的资金账户类型 枚举值： UNSETTLED : 未结算资金
     * AVAILABLE : 可用余额 UNAVAILABLE : 不可用余额 OPERATION : 运营户 BASIC :
     * 基本账户（含可用余额和不可用余额） 示例值：UNSETTLED
     */
