...
@RestController
@RequestMapping("/commons/open-dept")
@Slf4j
public class OpenDeptController extends BaseController {

    // @formatter:off

    private final OpenDeptService openDeptService;
    ...
    public RestResponse<List<OpenDept>> list(OpenDeptSearch search) {

        List<OpenDept> result = openDeptService.query(search);
    ...
    public RestResponse<IPage<OpenDept>> listPage(@ModelAttribute OpenDeptSearch search) {
        int current = 1;
        int size = 10;
        ...
        IPage<OpenDept> page = new Page<>(current, size);

        page = openDeptService.queryPage(page, search);
    ...
    public RestResponse<OpenDept> detail(@PathVariable("id") String id) {

        OpenDept result = openDeptService.get(id);
    ...
    public RestResponse<OpenDept> add(@RequestBody OpenDept add) {

        openDeptService.insert(add);
    ...
    public RestResponse<OpenDept> update(
            @PathVariable("id") String id, @RequestBody OpenDept update) {

        update.setId(id);

        openDeptService.update(update);
    ...
    public RestResponse<OpenDept> delete(@PathVariable("id") String id) {

        openDeptService.delete(id);
