...
@RestController
@RequestMapping("/commons/sys-process-form")
@Slf4j
public class SysProcessFormController extends BaseController {

    private final SysProcessFormService sysProcessFormService;
    ...
    public RestResponse<List<SysProcessForm>> list(SysProcessFormSearch search) {

        List<SysProcessForm> result = sysProcessFormService.query(search);
    ...
    public RestResponse<IPage<SysProcessForm>> listPage(
            @ModelAttribute SysProcessFormSearch search) {
        int current = 1;
        int size = 10;
        ...
        IPage<SysProcessForm> page = new Page<>(current, size);

        page = sysProcessFormService.queryPage(page, search);
    ...
    public RestResponse<SysProcessForm> detail(@PathVariable("id") String id) {

        SysProcessForm result = sysProcessFormService.get(id);
    ...
    public RestResponse<SysProcessForm> add(@RequestBody SysProcessForm add) {

        sysProcessFormService.insert(add);
    ...
    public RestResponse<SysProcessForm> update(
            @PathVariable("id") String id, @RequestBody SysProcessForm update) {

        update.setId(id);

        sysProcessFormService.update(update);
    ...
    public RestResponse<SysProcessForm> delete(@PathVariable("id") String id) {

        sysProcessFormService.delete(id);
