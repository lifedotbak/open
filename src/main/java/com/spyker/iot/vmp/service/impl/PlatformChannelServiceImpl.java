...
import com.spyker.iot.vmp.service.IPlatformChannelService;
import com.spyker.iot.vmp.storager.dao.DeviceChannelMapper;
import com.spyker.iot.vmp.storager.dao.ParentPlatformMapper;
import com.spyker.iot.vmp.storager.dao.PlatformCatalogMapper;
...
public class PlatformChannelServiceImpl implements IPlatformChannelService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(PlatformChannelServiceImpl.class);
    ...
    @Autowired private PlatformCatalogMapper catalogManager;
    @Autowired private ParentPlatformMapper platformMapper;

    @Override
    public int updateChannelForGB(
            String platformId, List<ChannelReduce> channelReduces, String catalogId) {
        ...
        List<Integer> channelIds =
                platformChannelMapper.findChannelRelatedPlatform(platformId, channelReduces);
        ...
        if (channelReducesToAdd.size() > 0) {
            if (channelReducesToAdd.size() > limitCount) {
                for (int i = 0; i < channelReducesToAdd.size(); i += limitCount) {
                    ...
                    int count =
                            platformChannelMapper.addChannels(
                                    platformId, channelReducesToAdd.subList(i, toIndex));
                    result = result || count < 0;
                    allCount += count;
                    logger.info(
                            "[关联通道]国标通道 平台：{}, 共需关联通道数:{}, 已关联：{}",
                            platformId,
                            channelReducesToAdd.size(),
                            toIndex);
                }
            } else {
            ...
            if (catalogSubscribe != null) {
                List<DeviceChannel> deviceChannelList =
                        getDeviceChannelListByChannelReduceList(
                                channelReducesToAdd, catalogId, platform);
                if (deviceChannelList != null) {
                    eventPublisher.catalogEventPublish(
                            platformId, deviceChannelList, CatalogEvent.ADD);
                }
            }
    ...
    private List<DeviceChannel> getDeviceChannelListByChannelReduceList(
            List<ChannelReduce> channelReduces, String catalogId, ParentPlatform platform) {
        List<DeviceChannel> deviceChannelList = new ArrayList<>();
        if (channelReduces.size() > 0) {
            PlatformCatalog catalog =
                    catalogManager.selectByPlatFormAndCatalogId(
                            platform.getServerGBId(), catalogId);
            if (catalog == null && catalogId.equals(platform.getDeviceGBId())) {
                for (ChannelReduce channelReduce : channelReduces) {
                    DeviceChannel deviceChannel =
                            deviceChannelMapper.queryChannel(
                                    channelReduce.getDeviceId(), channelReduce.getChannelId());
    ...
    public int delAllChannelForGB(String platformId, String catalogId) {
        ...
        List<DeviceChannel> deviceChannels =
                platformChannelMapper.queryAllChannelInCatalog(platformId, catalogId);
