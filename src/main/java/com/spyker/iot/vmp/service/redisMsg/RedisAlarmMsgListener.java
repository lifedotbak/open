...
public class RedisAlarmMsgListener implements MessageListener {
    ...
    private static final Logger logger = LoggerFactory.getLogger(RedisAlarmMsgListener.class);
    ...
    public void onMessage(@NotNull Message message, byte[] bytes) {
        // 消息示例：  PUBLISH alarm_receive '{ "gbId": "", "alarmSn": 1, "alarmType": "111",
        // "alarmDescription": "222", }'
        logger.info("收到来自REDIS的ALARM通知： {}", new String(message.getBody()));
        boolean isEmpty = taskQueue.isEmpty();
        taskQueue.offer(message);
        if (isEmpty) {
            //            logger.info("[线程池信息]活动线程数：{}, 最大线程数： {}", taskExecutor.getActiveCount(),
                        ...
                        while (!taskQueue.isEmpty()) {
                            ...
                            try {
                                AlarmChannelMessage alarmChannelMessage =
                                        JSON.parseObject(msg.getBody(), AlarmChannelMessage.class);
                                ...
                                deviceAlarm.setAlarmDescription(
                                        alarmChannelMessage.getAlarmDescription());
                                ...
                                if (ObjectUtils.isEmpty(gbId)) {
                                    if (userSetting.getSendToPlatformsWhenIdLost()) {
                                        ...
                                        List<ParentPlatform> parentPlatforms =
                                                storage.queryEnableParentPlatformList(true);
                                        if (parentPlatforms.size() > 0) {
                                            for (ParentPlatform parentPlatform : parentPlatforms) {
                                                try {
                                                    deviceAlarm.setChannelId(
                                                            parentPlatform.getDeviceGBId());
                                                    commanderForPlatform.sendAlarmMessage(
                                                            parentPlatform, deviceAlarm);
                                                } catch (SipException
                                        ...
                                        List<ParentPlatform> parentPlatforms =
                                                storage
                                                        .queryEnablePlatformListWithAsMessageChannel();
                                        if (parentPlatforms.size() > 0) {
                                            for (ParentPlatform parentPlatform : parentPlatforms) {
                                                try {
                                                    deviceAlarm.setChannelId(
                                    ...
                                    // 获取开启了消息推送的设备和平台
                                    List<Device> devices =
                                            storage.queryDeviceWithAsMessageChannel();
                                    if (devices.size() > 0) {
                                    ...
                                    ParentPlatform platform =
                                            storage.queryParentPlatByServerGBId(gbId);
