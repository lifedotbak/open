...
public class RedisGpsMsgListener implements MessageListener {
    ...
    private static final Logger logger = LoggerFactory.getLogger(RedisGpsMsgListener.class);
                        ...
                        while (!taskQueue.isEmpty()) {
                            ...
                            try {
                                GPSMsgInfo gpsMsgInfo =
                                        JSON.parseObject(msg.getBody(), GPSMsgInfo.class);
                                logger.info("[REDIS的位置变化通知], {}", JSON.toJSONString(gpsMsgInfo));
                                // 只是放入redis缓存起来
                                redisCatchStorage.updateGpsMsgInfo(gpsMsgInfo);
                            } catch (Exception e) {
                                logger.warn(
                                        "[REDIS的位置变化通知] 发现未处理的异常, \r\n{}",
