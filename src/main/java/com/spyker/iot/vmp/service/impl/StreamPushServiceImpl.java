...
import com.spyker.iot.vmp.gb28181.bean.PlatformCatalog;
...
import com.spyker.iot.vmp.service.IGbStreamService;
import com.spyker.iot.vmp.service.IStreamPushService;
import com.spyker.iot.vmp.service.bean.StreamPushItemFromRedis;
...
public class StreamPushServiceImpl implements IStreamPushService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(StreamPushServiceImpl.class);
    ...
    @Autowired private ParentPlatformMapper parentPlatformMapper;
    ...
    public void onApplicationEvent(MediaArrivalEvent event) {
        ...
        StreamAuthorityInfo streamAuthorityInfo =
                redisCatchStorage.getStreamAuthorityInfo(event.getApp(), event.getStream());
        ...
        redisCatchStorage.updateStreamAuthorityInfo(
                event.getApp(), event.getStream(), streamAuthorityInfo);
        ...
        //            try {
        //                channelOnlineEventLister.run(transform.getApp(), transform.getStream(),
        // transform.getServerId());;
        ...
        redisCatchStorage.sendStreamChangeMsg(
                OriginType.values()[event.getMediaInfo().getOriginType()].getType(), jsonObject);
    }
    ...
    public void onApplicationEvent(MediaDepartureEvent event) {
        ...
        if (mediaInfo != null) {
            ...
            redisCatchStorage.removeStream(
                    event.getMediaServer().getId(), type, event.getApp(), event.getStream());
            if ("PUSH".equalsIgnoreCase(type)) {
                // 冗余数据，自己系统中自用
                redisCatchStorage.removePushListItem(
                        event.getApp(), event.getStream(), event.getMediaServer().getId());
            }
            if (type != null) {
                // 发送流变化redis消息
    ...
    //    @Override
    //    public PageInfo<StreamPushItem> getPushList(
    //            Integer page, Integer count, String query, Boolean pushing, String mediaServerId)
    // {
    //        PageHelper.startPage(page, count);
    //        List<StreamPushItem> all = streamPushMapper.selectAllForList(query, pushing,
    // mediaServerId);
    ...
    public boolean removeFromGB(GbStream stream) {
        ...
        List<StreamInfo> mediaList =
                mediaServerService.getMediaList(
                        mediaInfo, stream.getApp(), stream.getStream(), null);
    ...
    public boolean stop(String app, String streamId) {
        ...
        if (delStream > 0) {
            MediaServer mediaServerItem =
                    mediaServerService.getOne(streamPushItem.getMediaServerId());
    ...
    public void zlmServerOnline(String mediaServerId) {
        ...
        if (pushList.size() > 0) {
            for (StreamPushItem streamPushItem : pushList) {
                if (ObjectUtils.isEmpty(streamPushItem.getGbId())) {
                    pushItemMap.put(
                            streamPushItem.getApp() + streamPushItem.getStream(), streamPushItem);
                }
            }
        }
        ...
        // 获取所有推流鉴权信息，清理过期的
        List<StreamAuthorityInfo> allStreamAuthorityInfo =
                redisCatchStorage.getAllStreamAuthorityInfo();
        ...
        for (StreamAuthorityInfo streamAuthorityInfo : allStreamAuthorityInfo) {
            streamAuthorityInfoInfoMap.put(
                    streamAuthorityInfo.getApp() + streamAuthorityInfo.getStream(),
                    streamAuthorityInfo);
        }
        List<StreamInfo> mediaList =
                mediaServerService.getMediaList(mediaServerItem, null, null, null);
        if (mediaList == null) {
            return;
        }
        ...
        if (streamPushItems != null) {
            for (StreamPushItem streamPushItem : streamPushItems) {
                ...
                streamAuthorityInfoInfoMap.remove(
        ...
        if (mediaInfos.size() > 0) {
            ...
            for (MediaInfo mediaInfo : mediaInfos) {
                ...
                // 移除redis内流的信息
                redisCatchStorage.removeStream(
                        mediaServerItem.getId(), "PUSH", mediaInfo.getApp(), mediaInfo.getStream());
                // 冗余数据，自己系统中自用
                redisCatchStorage.removePushListItem(
                        mediaInfo.getApp(), mediaInfo.getStream(), mediaServerItem.getId());
            }
        }
        ...
        if (streamAuthorityInfos.size() > 0) {
            for (StreamAuthorityInfo streamAuthorityInfo : streamAuthorityInfos) {
                // 移除redis内流的信息
                redisCatchStorage.removeStreamAuthorityInfo(
                        streamAuthorityInfo.getApp(), streamAuthorityInfo.getStream());
            }
        }
    }
    ...
    public void zlmServerOffline(String mediaServerId) {
        List<StreamPushItem> streamPushItems =
                streamPushMapper.selectAllByMediaServerIdWithOutGbID(mediaServerId);
        // 移除没有GBId的推流
        ...
        if (mediaInfoList.size() > 0) {
            for (MediaInfo mediaInfo : mediaInfoList) {
                // 移除redis内流的信息
                redisCatchStorage.removeStream(
                        mediaServerId, type, mediaInfo.getApp(), mediaInfo.getStream());
                JSONObject jsonObject = new JSONObject();
                jsonObject.put("serverId", userSetting.getServerId());
                jsonObject.put("app", mediaInfo.getApp());
                jsonObject.put("stream", mediaInfo.getStream());
                ...
                // 冗余数据，自己系统中自用
                redisCatchStorage.removePushListItem(
                        mediaInfo.getApp(), mediaInfo.getStream(), mediaServerId);
            }
        }
    }
    ...
    @Override
    public void batchAddForUpload(
            List<StreamPushItem> streamPushItems,
            Map<String, List<String[]>> streamPushItemsForAll) {
        // 存储数据到stream_push表
        streamPushMapper.addAll(streamPushItems);
        List<StreamPushItem> streamPushItemForGbStream =
                streamPushItems.stream()
        ...
        List<StreamPushItem> streamPushItemsForPlatform =
                streamPushItemForGbStream.stream()
        ...
        if (streamPushItemsForPlatform.size() > 0) {
            ...
            for (ParentPlatform platform : parentPlatformList) {
                ...
                List<PlatformCatalog> platformCatalogs =
                        platformCatalogMapper.selectByPlatForm(platform.getServerGBId());
            ...
            for (StreamPushItem streamPushItem : streamPushItemsForPlatform) {
                List<String[]> platFormInfoList =
                        streamPushItemsForAll.get(
                                streamPushItem.getApp() + streamPushItem.getStream());
                    ...
                    for (String[] platFormInfoArray : platFormInfoList) {
                        ...
                        if (platFormInfoArray.length > 0) {
                            ...
                            if (platformInfoMap.get(platFormInfoArray[0]) == null
                                    || platformInfoMap
                                                    .get(platFormInfoArray[0])
                                                    .get(platFormInfoArray[1])
                                            == null) {
                                logger.info(
                                        "导入数据时不存在平台或目录{}/{},已导入未分配",
                                        platFormInfoArray[0],
                                        platFormInfoArray[1]);
                                continue;
                            }
                            streamPushItemForPlatform.setPlatformId(platFormInfoArray[0]);
                            List<GbStream> gbStreamList =
                                    platformForEvent.get(platFormInfoArray[0]);
    ...
    public boolean batchStop(List<GbStream> gbStreams) {
        ...
        if (delStream > 0) {
            for (GbStream gbStream : gbStreams) {
                MediaServer mediaServerItem =
                        mediaServerService.getOne(gbStream.getMediaServerId());
                mediaServerService.closeStreams(
