...
/**
 * @Auther: JiangFeng @Date: 2022/8/16 11:32 @Description: 接收redis发送的推流设备列表更新通知
 */
@Component
                        ...
                        while (!taskQueue.isEmpty()) {
                            ...
                            try {
                                List<StreamPushItem> streamPushItems =
                                        JSON.parseArray(
                                                new String(msg.getBody()), StreamPushItem.class);
                                // 查询全部的app+stream 用于判断是添加还是修改
                                Map<String, StreamPushItem> allAppAndStream =
                                        streamPushService.getAllAppAndStreamMap();
                                ...
                                for (StreamPushItem streamPushItem : streamPushItems) {
                                    ...
                                    if (!contains) {
                                        if (allGBId.containsKey(streamPushItem.getGbId())) {
                                            GbStream gbStream =
                                                    allGBId.get(streamPushItem.getGbId());
                                            logger.warn(
                                                    "[REDIS消息-推流设备列表更新-INSERT] 国标编号重复: {}, 已分配给{}/{}",
                                                    streamPushItem.getGbId(),
                                                    gbStream.getApp(),
                                                    gbStream.getStream());
                                            continue;
                                        }
                                        ...
                                        streamPushItem.setMediaServerId(
                                        ...
                                        if (allGBId.containsKey(streamPushItem.getGbId())
                                            ...
                                            GbStream gbStream =
                                                    allGBId.get(streamPushItem.getGbId());
                                            logger.warn(
                                                    "[REDIS消息-推流设备列表更新-UPDATE] 国标编号重复: {}, 已分配给{}/{}",
                                                    streamPushItem.getGbId(),
                                                    gbStream.getApp(),
                                                    gbStream.getStream());
                                            continue;
