...
public class GbStreamServiceImpl implements IGbStreamService {
    ...
    public boolean addPlatformInfo(List<GbStream> gbStreams, String platformId, String catalogId) {
        ...
        try {
            ...
            for (int i = 0; i < gbStreams.size(); i++) {
                ...
                logger.info(
                        "[关联通道]直播流通道 平台：{}, 共需关联通道数:{}, 已关联：{}",
                        platformId,
                        gbStreams.size(),
                        i + 1);
                DeviceChannel deviceChannelListByStream =
                        getDeviceChannelListByStreamWithStatus(gbStream, catalogId, parentPlatform);
    ...
    @Override
    public DeviceChannel getDeviceChannelListByStream(
            GbStream gbStream, String catalogId, ParentPlatform platform) {
        ...
        PlatformCatalog catalog =
                catalogMapper.selectByPlatFormAndCatalogId(platform.getServerGBId(), catalogId);
        if (catalog != null) {
    ...
    public void sendCatalogMsg(GbStream gbStream, String type) {
        ...
        if (gbStream.getGbId() != null) {
            ...
            GbStream gbStreamIndb =
                    gbStreamMapper.selectOne(gbStream.getApp(), gbStream.getStream());
        ...
        if (gbStreams.size() > 0) {
            for (GbStream gs : gbStreams) {
                ...
                List<ParentPlatform> parentPlatforms =
                        platformGbStreamMapper.selectByAppAndStream(gs.getApp(), gs.getStream());
                if (parentPlatforms.size() > 0) {
                    for (ParentPlatform parentPlatform : parentPlatforms) {
                        if (parentPlatform != null) {
                            eventPublisher.catalogEventPublishForStream(
                                    parentPlatform.getServerGBId(), gs, type);
                        }
                    }
                }
            }
    ...
    @Override
    public DeviceChannel getDeviceChannelListByStreamWithStatus(
            GbStream gbStream, String catalogId, ParentPlatform platform) {
        DeviceChannel deviceChannel = new DeviceChannel();
        deviceChannel.setChannelId(gbStream.getGbId());
        deviceChannel.setName(gbStream.getName());
