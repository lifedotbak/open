...
import com.spyker.iot.vmp.vmanager.bean.StreamPushExcelDto;
...
public class StreamPushUploadFileHandler extends AnalysisEventListener<StreamPushExcelDto> {
    ...
    public StreamPushUploadFileHandler(
            IStreamPushService pushService,
            String defaultMediaServerId,
    ...
    public void invoke(StreamPushExcelDto streamPushExcelDto, AnalysisContext analysisContext) {
        ...
        if (gBMap.get(streamPushExcelDto.getApp() + streamPushExcelDto.getStream()) == null) {
            try {
                gBMap.put(
                        streamPushExcelDto.getApp() + streamPushExcelDto.getStream(),
                        streamPushExcelDto.getGbId());
            } catch (IllegalArgumentException e) {
                errorInfoList.add(
                        "行：" + rowIndex + ", " + streamPushExcelDto.getGbId() + " 国标ID重复使用");
                return;
            }
        } else {
            if (!gBMap.get(streamPushExcelDto.getApp() + streamPushExcelDto.getStream())
                    .equals(streamPushExcelDto.getGbId())) {
                errorInfoList.add(
                        "行："
                                + rowIndex
                                + ", "
                                + streamPushExcelDto.getGbId()
                                + " 同样的应用名和流ID使用了不同的国标ID");
                return;
            }
        }

        if (streamPushStreamSet.contains(
                streamPushExcelDto.getApp()
                        + streamPushExcelDto.getStream()
                        + streamPushExcelDto.getPlatformId())) {
            errorStreamList.add(
                    "行："
                            + rowIndex
                            + ", "
            ...
            if (pushMapInDb.get(streamPushExcelDto.getApp() + streamPushExcelDto.getStream())
                    != null) {
                errorStreamList.add(
                        "行："
                                + rowIndex
    ...
    private void saveData() {
        if (streamPushItemForSave.size() > 0) {
            // 向数据库查询是否存在重复的app
            pushService.batchAddForUpload(
                    new ArrayList<>(streamPushItemForSave.values()), streamPushItemsForPlatform);
        }
    }
