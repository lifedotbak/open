package com.spyker.iot.vmp.jt1078.codec.encode;
...
import com.spyker.iot.vmp.jt1078.util.Bin;
...
public class Jt808EncoderCmd extends MessageToByteEncoder<Cmd> {
    private static final Logger log = LoggerFactory.getLogger(Jt808EncoderCmd.class);
    ...
    public static ByteBuf encode(Rs msg, Session session, Integer packageNo) {
        ...
        if (header.is2019Version()) {
            ...
            // 终端手机号
            byteBuf.writeBytes(
                    ByteBufUtil.decodeHexDump(Bin.strHexPaddingLeft(header.getDevId(), 20)));
        } else {
            // 消息体属性
            byteBuf.writeShort(encode.readableBytes());

            byteBuf.writeBytes(
                    ByteBufUtil.decodeHexDump(Bin.strHexPaddingLeft(header.getDevId(), 12)));
        }
    ...
    public static byte bcc(ByteBuf byteBuf) {
        byte cs = 0;
        while (byteBuf.isReadable()) cs ^= byteBuf.readByte();
