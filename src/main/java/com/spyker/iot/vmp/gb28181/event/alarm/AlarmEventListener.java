...
public class AlarmEventListener implements ApplicationListener<AlarmEvent> {
    ...
    public void onApplicationEvent(@NotNull AlarmEvent event) {
        ...
        String msg =
                "<strong>设备编号：</strong> <i>"
                        + event.getAlarmInfo().getDeviceId()
                        + "</i>"
                        + "<br><strong>通道编号：</strong> <i>"
                        + event.getAlarmInfo().getChannelId()
                        + "</i>"
                        + "<br><strong>报警描述：</strong> <i>"
                        + event.getAlarmInfo().getAlarmDescription()
                        + "</i>"
                        + "<br><strong>报警时间：</strong> <i>"
                        + event.getAlarmInfo().getAlarmTime()
                        + "</i>";

        for (Iterator<Map.Entry<String, PrintWriter>> it = SSE_CACHE.entrySet().iterator();
                it.hasNext(); ) {
