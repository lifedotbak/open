package com.spyker.iot.vmp.gb28181.transmit.event.response.impl;
...
import com.spyker.iot.vmp.gb28181.bean.SubscribeHolder;
...
import com.spyker.iot.vmp.gb28181.transmit.event.response.SIPResponseProcessorAbstract;
...
public class RegisterResponseProcessor extends SIPResponseProcessorAbstract {
    ...
    public void process(ResponseEvent evt) {
        ...
        PlatformRegisterInfo platformRegisterInfo =
                redisCatchStorage.queryPlatformRegisterInfo(callId);
        ...
        ParentPlatformCatch parentPlatformCatch =
                redisCatchStorage.queryPlatformCatchInfo(platformRegisterInfo.getPlatformId());
        if (parentPlatformCatch == null) {
            logger.warn(
                    String.format(
                            "[国标级联]收到注册/注销%S请求，平台：%s，但是平台缓存信息未查询到!!!",
                            response.getStatusCode(), platformRegisterInfo.getPlatformId()));
            return;
        }
        ...
        logger.info(
                String.format(
                        "[国标级联]%s %S响应,%s ",
                        action, response.getStatusCode(), platformRegisterInfo.getPlatformId()));
        ParentPlatform parentPlatform = parentPlatformCatch.getParentPlatform();
        if (parentPlatform == null) {
            logger.warn(
                    String.format(
                            "[国标级联]收到 %s %s的%S请求, 但是平台信息未查询到!!!",
                            platformRegisterInfo.getPlatformId(),
                            action,
        ...
        if (response.getStatusCode() == Response.UNAUTHORIZED) {
            ...
            try {
                sipCommanderForPlatform.register(
                        parentPlatform,
                        sipTransactionInfo,
                        www,
                        null,
                        null,
                        platformRegisterInfo.isRegister());
