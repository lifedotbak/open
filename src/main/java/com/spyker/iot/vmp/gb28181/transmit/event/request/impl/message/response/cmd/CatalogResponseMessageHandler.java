...
import com.spyker.iot.vmp.gb28181.session.CatalogDataCatch;
...
/** 目录查询的回复 */
@Component
public class CatalogResponseMessageHandler extends SIPRequestProcessorParent
                        ...
                        while (!taskQueue.isEmpty()) {
                            ...
                            try {
                                ...
                                try {
                                    rootElement =
                                            getRootElement(
                                                    take.getEvt(), take.getDevice().getCharset());
                                ...
                                if (sumNum == 0) {
                                    ...
                                    catalogDataCatch.setChannelSyncEnd(
                                            take.getDevice().getDeviceId(), null);
                                } else {
                                    Iterator<Element> deviceListIterator =
                                            deviceListElement.elementIterator();
                                    if (deviceListIterator != null) {
                                        ...
                                        while (deviceListIterator.hasNext()) {
                                            ...
                                            Element channelDeviceElement =
                                                    itemDevice.element("DeviceID");
                                            if (channelDeviceElement == null) {
                                                continue;
                                            }
                                            DeviceChannel channel =
                                                    XmlUtil.channelContentHandler(
                                                            itemDevice, device, null);
                                            if (channel == null) {
                                                logger.info(
                                                        "[收到目录订阅]：但是解析失败 {}",
                                                        new String(
                                                                evt.getRequest().getRawContent()));
                                                continue;
                                            }
                                            if (channel.getParentId() != null
                                                    && channel.getParentId()
                                                            .equals(sipConfig.getId())) {
                                        ...
                                        catalogDataCatch.put(
                                                take.getDevice().getDeviceId(),
                                                sn,
                                                sumNum,
                                                take.getDevice(),
                                                channelList);
                                        logger.info(
                                                ...
                                                take.getDevice().getDeviceId(),
                                                channelList.size(),
                                                catalogDataCatch.get(take.getDevice().getDeviceId())
                                                                == null
                                                        ? 0
                                                        : catalogDataCatch
                                                                .get(take.getDevice().getDeviceId())
                                                                .size(),
                                                sumNum);
                                        if (catalogDataCatch
                                                        .get(take.getDevice().getDeviceId())
                                            ...
                                            boolean resetChannelsResult =
                                                    storager.resetChannels(
                                                            take.getDevice().getDeviceId(),
                                                            catalogDataCatch.get(
                                                                    take.getDevice()
                                                                            .getDeviceId()));
                                            if (!resetChannelsResult) {
                                                String errorMsg =
                                                        "接收成功，写入失败，共"
                                                                ...
                                                                + "条";
                                                catalogDataCatch.setChannelSyncEnd(
                                                        take.getDevice().getDeviceId(), errorMsg);
                                            } else {
                                                catalogDataCatch.setChannelSyncEnd(
                                                        take.getDevice().getDeviceId(), null);
                                            }
