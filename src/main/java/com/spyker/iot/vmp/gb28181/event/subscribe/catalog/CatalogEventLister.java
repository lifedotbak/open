package com.spyker.iot.vmp.gb28181.event.subscribe.catalog;
...
public class CatalogEventLister implements ApplicationListener<CatalogEvent> {
    ...
    private static final Logger logger = LoggerFactory.getLogger(CatalogEventLister.class);

    @Autowired private IVideoManagerStorage storager;

    @Autowired private SIPCommanderFroPlatform sipCommanderFroPlatform;
            ...
            if (event.getDeviceChannels() != null) {
                if (platforms.size() > 0) {
                    for (DeviceChannel deviceChannel : event.getDeviceChannels()) {
                        List<ParentPlatform> parentPlatformsForGB =
                                storager.queryPlatFormListForGBWithGBId(
                                        deviceChannel.getChannelId(), platforms);
                ...
                if (platforms.size() > 0) {
                    for (GbStream gbStream : event.getGbStreams()) {
                        ...
                        List<ParentPlatform> parentPlatformsForGB =
                                storager.queryPlatFormListForStreamWithGBId(
                                        gbStream.getApp(), gbStream.getStream(), platforms);
                ...
                if (parentPlatform != null || subscribe != null) {
                    ...
                    if (event.getGbStreams() != null && event.getGbStreams().size() > 0) {
                        for (GbStream gbStream : event.getGbStreams()) {
                            ...
                            DeviceChannel deviceChannelByStream =
                                    gbStreamService.getDeviceChannelListByStream(
                                            gbStream, gbStream.getCatalogId(), parentPlatform);
                    ...
                    if (deviceChannelList.size() > 0) {
                        ...
                        try {
                            sipCommanderFroPlatform.sendNotifyForCatalogOther(
                                    event.getType(),
                                    parentPlatform,
                                    deviceChannelList,
                                    subscribe,
                        ...
                        if (parentPlatforms != null && parentPlatforms.size() > 0) {
                            for (ParentPlatform platform : parentPlatforms) {
                                SubscribeInfo subscribeInfo =
                                        subscribeHolder.getCatalogSubscribe(
                ...
                if (parentPlatform != null || subscribe != null) {
                    ...
                    if (event.getGbStreams() != null && !event.getGbStreams().isEmpty()) {
                        for (GbStream gbStream : event.getGbStreams()) {
                            deviceChannelList.add(
                                    gbStreamService.getDeviceChannelListByStreamWithStatus(
                                            gbStream, gbStream.getCatalogId(), parentPlatform));
                        }
                    }
                    if (!deviceChannelList.isEmpty()) {
                        logger.info(
                                ...
                                deviceChannelList.size());
                        try {
                            sipCommanderFroPlatform.sendNotifyForCatalogAddOrUpdate(
                                    event.getType(),
                                    parentPlatform,
                                    deviceChannelList,
                                    subscribe,
                            ...
                            for (ParentPlatform platform : parentPlatforms) {
                                ...
                                DeviceChannel deviceChannel =
                                        storager.queryChannelInParentPlatform(
                                                platform.getServerGBId(), gbId);
                                deviceChannelList.add(deviceChannel);
                                GbStream gbStream =
                                        storager.queryStreamInParentPlatform(
                                                platform.getServerGBId(), gbId);
                                if (gbStream != null) {
                                    DeviceChannel deviceChannelByStream =
                                            gbStreamService.getDeviceChannelListByStreamWithStatus(
                                                    gbStream, gbStream.getCatalogId(), platform);
                                    deviceChannelList.add(deviceChannelByStream);
                                }
                                try {
                                    sipCommanderFroPlatform.sendNotifyForCatalogAddOrUpdate(
