...
import com.spyker.iot.vmp.gb28181.bean.CatalogData;
...
public class CatalogDataCatch {
    ...
    public void addReady(Device device, int sn) {
        ...
        if (catalogData == null
                || catalogData.getStatus().equals(CatalogData.CatalogDataStatus.end)) {
    ...
    private void timerTask() {
        ...
        for (String deviceId : keys) {
                ...
                if (catalogData.getStatus().equals(CatalogData.CatalogDataStatus.runIng)) {
                    if (catalogData.getTotal() == catalogData.getChannelList().size()) {
                        storager.resetChannels(
                                catalogData.getDevice().getDeviceId(),
                                catalogData.getChannelList());
                    } else {
                        storager.updateChannels(
                                catalogData.getDevice().getDeviceId(),
                                catalogData.getChannelList());
                    }
                    String errorMsg =
                            "更新成功，共"
                                    + catalogData.getTotal()
                                    + "条，已更新"
                                    + catalogData.getChannelList().size()
                                    + "条";
            ...
            if (catalogData.getStatus().equals(CatalogData.CatalogDataStatus.end)
                    && catalogData.getLastTime().isBefore(instantBefore30S)) { // 超过三十秒，如果标记为end则删除
