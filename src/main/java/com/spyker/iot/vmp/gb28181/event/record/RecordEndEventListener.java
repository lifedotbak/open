package com.spyker.iot.vmp.gb28181.event.record;
...
public class RecordEndEventListener implements ApplicationListener<RecordEndEvent> {
    ...
    private static final Logger logger = LoggerFactory.getLogger(RecordEndEventListener.class);
    ...
    public void onApplicationEvent(RecordEndEvent event) {
        ...
        logger.info(
                "录像查询完成事件触发，deviceId：{}, channelId: {}, 录像数量{}/{}条",
                event.getRecordInfo().getDeviceId(),
                event.getRecordInfo().getChannelId(),
                count,
                sumNum);
        if (!handlerMap.isEmpty()) {
            RecordEndEventHandler handler = handlerMap.get(deviceId + channelId);
            logger.info(
                    "录像查询完成事件触发, 发送订阅，deviceId：{}, channelId: {}",
                    event.getRecordInfo().getDeviceId(),
                    event.getRecordInfo().getChannelId());
            if (handler != null) {
     ...
     */
    public void addEndEventHandler(
            String device, String channelId, RecordEndEventHandler recordEndEventHandler) {
