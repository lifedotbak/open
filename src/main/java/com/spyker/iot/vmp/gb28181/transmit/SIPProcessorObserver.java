package com.spyker.iot.vmp.gb28181.transmit;
...
import com.spyker.iot.vmp.gb28181.transmit.event.response.ISIPResponseProcessor;
import com.spyker.iot.vmp.gb28181.transmit.event.timeout.ITimeoutProcessor;
...
public class SIPProcessorObserver implements ISIPProcessorObserver {
    ...
    private static Map<String, ISIPRequestProcessor> requestProcessorMap =
            new ConcurrentHashMap<>();
    private static Map<String, ISIPResponseProcessor> responseProcessorMap =
            new ConcurrentHashMap<>();
    ...
    public void processResponse(ResponseEvent responseEvent) {
        ...
        if (((status >= Response.OK) && (status < Response.MULTIPLE_CHOICES))
            ...
            CSeqHeader cseqHeader =
                    (CSeqHeader) responseEvent.getResponse().getHeader(CSeqHeader.NAME);
            ...
            if (status != Response.UNAUTHORIZED
                    ...
                    && sipSubscribe.getOkSubscribesSize() > 0) {
                CallIdHeader callIdHeader =
                        (CallIdHeader) responseEvent.getResponse().getHeader(CallIdHeader.NAME);
                if (callIdHeader != null) {
                    SipSubscribe.Event subscribe =
                            sipSubscribe.getOkSubscribe(callIdHeader.getCallId());
                    if (subscribe != null) {
                        SipSubscribe.EventResult eventResult =
                                new SipSubscribe.EventResult(responseEvent);
            ...
            logger.warn(
                    "接收到失败的response响应！status：" + status + ",message:" + response.getReasonPhrase());
            if (responseEvent.getResponse() != null && sipSubscribe.getErrorSubscribesSize() > 0) {
                CallIdHeader callIdHeader =
                        (CallIdHeader) responseEvent.getResponse().getHeader(CallIdHeader.NAME);
                if (callIdHeader != null) {
                    SipSubscribe.Event subscribe =
                            sipSubscribe.getErrorSubscribe(callIdHeader.getCallId());
                    if (subscribe != null) {
                        SipSubscribe.EventResult eventResult =
                                new SipSubscribe.EventResult(responseEvent);
                        subscribe.response(eventResult);
            ...
            if (request != null) {
                ...
                if (callIdHeader != null) {
                    ...
                    SipSubscribe.Event subscribe =
                            sipSubscribe.getErrorSubscribe(callIdHeader.getCallId());
                    SipSubscribe.EventResult eventResult =
                            new SipSubscribe.EventResult(timeoutEvent);
                    if (subscribe != null) {
                        subscribe.response(eventResult);
                    }
                    sipSubscribe.removeOkSubscribe(callIdHeader.getCallId());
    ...
    public void processTransactionTerminated(
        ...
        //        CallIdHeader header =
        // (CallIdHeader)transaction.getRequest().getHeader(CallIdHeader.NAME);
        //        SipSubscribe.EventResult<TransactionTerminatedEvent> terminatedEventEventResult =
        // new SipSubscribe.EventResult<>(transactionTerminatedEvent);
