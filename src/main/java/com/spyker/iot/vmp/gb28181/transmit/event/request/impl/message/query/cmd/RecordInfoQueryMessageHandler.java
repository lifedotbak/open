...
import com.spyker.iot.vmp.gb28181.event.record.RecordEndEventListener;
...
public class RecordInfoQueryMessageHandler extends SIPRequestProcessorParent
    ...
    @Autowired private SIPCommander commander;

    @Autowired private RecordEndEventListener recordEndEventListener;
    ...
    public void handForPlatform(
        ...
        List<ChannelSourceInfo> channelSources =
                storager.getChannelSource(parentPlatform.getServerGBId(), channelId);
        ...
        if (channelSources.get(0).getCount() > 0) { // 国标
            ...
            DeviceChannel deviceChannel =
                    storager.queryChannelInParentPlatform(
                            parentPlatform.getServerGBId(), channelId);
            // 接收录像数据
            recordEndEventListener.addEndEventHandler(
                    deviceChannel.getDeviceId(),
                    channelId,
                    (recordInfo) -> {
                        try {
                            logger.info("[国标级联] 录像查询收到数据， 通道： {}，准备转发===", channelId);
                            cmderFroPlatform.recordInfo(
                                    deviceChannel,
                                    parentPlatform,
                                    request.getFromTag(),
                                    recordInfo);
            ...
            try {
                commander.recordInfoQuery(
                        device,
                        channelId,
                        DateUtil.ISO8601Toyyyy_MM_dd_HH_mm_ss(startTime),
