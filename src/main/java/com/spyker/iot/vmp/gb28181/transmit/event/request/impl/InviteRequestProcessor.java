...
import com.spyker.iot.vmp.service.bean.ErrorCallback;
import com.spyker.iot.vmp.service.bean.InviteErrorCode;
...
import com.spyker.iot.vmp.service.redisMsg.RedisPushStreamResponseListener;
...
public class InviteRequestProcessor extends SIPRequestProcessorParent
    ...
    @Autowired private ISIPCommanderForPlatform cmderFroPlatform;
    ...
    public void process(RequestEvent evt) {
        ...
        try {
            ...
            final String channelId =
                    StringUtils.isNotBlank(channelIdFromSdp) ? channelIdFromSdp : channelIdFromSub;
            ...
            if (platform == null) {
                ...
                DeviceChannel channel =
                        storager.queryChannelInParentPlatform(requesterId, channelId);
                ...
                if (channel != null && gbStream == null) {
                    ...
                    if (mediaServerItem == null) {
                        if ("proxy".equals(gbStream.getStreamType())) {
                            ...
                            streamPushItem =
                                    streamPushService.getPush(
                                            gbStream.getApp(), gbStream.getStream());
                            if (streamPushItem != null) {
                                mediaServerItem =
                                        mediaServerService.getOne(
                                                streamPushItem.getMediaServerId());
                            }
                            if (mediaServerItem == null) {
                        ...
                        if ("push".equals(gbStream.getStreamType())) {
                            ...
                            proxyByAppAndStream =
                                    streamProxyService.getStreamProxyByAppAndStream(
                                            gbStream.getApp(), gbStream.getStream());
                ...
                Instant start = null;
                Instant end = null;
                if (sdp.getTimeDescriptions(false) != null
                        && sdp.getTimeDescriptions(false).size() > 0) {
                    TimeDescriptionImpl timeDescription =
                            (TimeDescriptionImpl) (sdp.getTimeDescriptions(false).get(0));
                ...
                if (channel != null) {
                    device =
                            storager.queryVideoDeviceByPlatformIdAndChannelId(
                    ...
                    if (userSetting.getUseCustomSsrcForParentInvite()
                            || gb28181Sdp.getSsrc() == null) {
                        // 上级平台点播时不使用上级平台指定的ssrc，使用自定义的ssrc，参考国标文档-点播外域设备媒体流SSRC处理方式
                        ssrc =
                                "Play".equalsIgnoreCase(sessionName)
                    ...
                    SendRtpItem sendRtpItem =
                            mediaServerService.createSendRtpItem(
                                    mediaServerItem,
                                    addressStr,
                                    port,
                                    ssrc,
                    ...
                    sendRtpItem.setPlayType(
                            "Play".equalsIgnoreCase(sessionName)
                                    ? InviteStreamType.PLAY
                                    : InviteStreamType.PLAYBACK);

                    Long finalStartTime = startTime;
                    Long finalStopTime = stopTime;
                    ErrorCallback<Object> hookEvent =
                            (code, msg, data) -> {
                                StreamInfo streamInfo = (StreamInfo) data;
                                MediaServer mediaServerItemInUSe =
                                        mediaServerService.getOne(streamInfo.getMediaServerId());
                                logger.info(
                                        "[上级Invite]下级已经开始推流。 回复200OK(SDP)， {}/{}",
                                        streamInfo.getApp(),
                                ...
                                if ("Playback".equalsIgnoreCase(sessionName)) {
                                    content.append(
                                            "t=" + finalStartTime + " " + finalStopTime + "\r\n");
                                } else {
                                    ...
                                    dynamicTask.startDelay(
                                            ...
                                            () -> {
                                                ...
                                                mediaServerService.releaseSsrc(
                                                        mediaServerItemInUSe.getId(),
                                                        sendRtpItem.getSsrc());
                                                // 回复bye
                                                try {
                                                    cmderFroPlatform.streamByeCmd(
                                                            platform, callIdHeader.getCallId());
                                                } catch (SipException
                                                        | InvalidArgumentException
                                                        | ParseException e) {
                                                    logger.error(
                                    ...
                                    if (sendRtpItem.isTcpActive()) {
                                        ...
                                        try {
                                            mediaServerService.startSendRtpPassive(
                                                    mediaServer, platform, sendRtpItem, 5);
                    ...
                    ErrorCallback<Object> errorEvent =
                            ((statusCode, msg, data) -> {
                                logger.info(
                                        "[上级Invite] {}, 失败, 平台：{}， 通道：{}, code： {}， msg；{}",
                                        sessionName,
                                        username,
                                        channelId,
                                        statusCode,
                                ...
                                try {
                                    Response response =
                                            getMessageFactory()
                                                    .createResponse(statusCode, evt.getRequest());
                                    sipSender.transmitRequest(
                                            request.getLocalAddress().getHostAddress(), response);
                    ...
                    if ("Playback".equalsIgnoreCase(sessionName)) {
                        ...
                        // 写入redis， 超时时回复
                        redisCatchStorage.updateSendRTPSever(sendRtpItem);
                        playService.playBack(
                                mediaServerItem,
                                ssrcInfo,
                                device.getDeviceId(),
                                channelId,
                                DateUtil.formatter.format(start),
                                DateUtil.formatter.format(end),
                                (code, msg, data) -> {
                                    if (code == InviteErrorCode.SUCCESS.getCode()) {
                                        hookEvent.run(code, msg, data);
                                    } else if (code
                                                    == InviteErrorCode.ERROR_FOR_SIGNALLING_TIMEOUT
                                                            .getCode()
                                            || code
                                                    == InviteErrorCode.ERROR_FOR_STREAM_TIMEOUT
                                                            .getCode()) {
                        ...
                        playService.download(
                                ...
                                DateUtil.formatter.format(start),
                                DateUtil.formatter.format(end),
                                Integer.parseInt(downloadSpeed),
                                (code, msg, data) -> {
                                    if (code == InviteErrorCode.SUCCESS.getCode()) {
                                        hookEvent.run(code, msg, data);
                                    } else if (code
                    ...
                    if ("push".equals(gbStream.getStreamType())) {
                        if (streamPushItem != null) {
                            // 从redis查询是否正在接收这个推流
                            StreamPushItem pushListItem =
                                    redisCatchStorage.getPushListItem(
                                            gbStream.getApp(), gbStream.getStream());
                            if (pushListItem != null) {
                                pushListItem.setSelf(
                                        userSetting
                                                .getServerId()
                                                .equals(pushListItem.getServerId()));
                                // 推流状态
                                pushStream(
                                        evt,
                                        request,
    ...
    private void startSendRtpStreamHand(
        ...
        if (jsonObject == null) {
            ...
            logger.info(
                    ...
                    param.get("app"),
                    param.get("stream"),
                    jsonObject.getString("local_port"),
                    param.get("dst_url"),
                    param.get("dst_port"));
        } else {
            logger.error(
    ...
    private void pushProxyStream(
        ...
        Boolean streamReady =
                mediaServerService.isStreamReady(
                        mediaServer, gbStream.getApp(), gbStream.getStream());
    ...
    private void pushStream(
        ...
        if (streamPushItem.isSelf()) {
            Boolean streamReady =
                    mediaServerService.isStreamReady(
                            mediaServerItem, gbStream.getApp(), gbStream.getStream());
            if (streamReady != null && streamReady) {
                // 自平台内容
                SendRtpItem sendRtpItem =
                        mediaServerService.createSendRtpItem(
                ...
                SIPResponse response =
                        sendStreamAck(mediaServerItem, request, sendRtpItem, platform, evt);
                if (response != null) {
                    sendRtpItem.setToTag(response.getToTag());
                }
                redisCatchStorage.updateSendRTPSever(sendRtpItem);
    ...
    /** 通知流上线 */
    private void notifyStreamOnline(
            RequestEvent evt,
            SIPRequest request,
            GbStream gbStream,
            StreamPushItem streamPushItem,
        ...
        if ("proxy".equals(gbStream.getStreamType())) {
            // TODO 控制启用以使设备上线
            logger.info(
                    "[ app={}, stream={} ]通道未推流，启用流后开始推流", gbStream.getApp(), gbStream.getStream());
            // 监听流上线
            Hook hook =
                    Hook.getInstance(
            ...
            this.hookSubscribe.addSubscribe(
                    hook,
                    (hookData) -> {
                        logger.info(
                                "[上级点播]拉流代理已经就绪， {}/{}", hookData.getApp(), hookData.getStream());
            ...
            if (!start) {
                try {
                    responseAck(
                            request,
                            Response.BUSY_HERE,
                            "channel [" + gbStream.getGbId() + "] offline");
                } catch (SipException | InvalidArgumentException | ParseException e) {
            ...
            if (!platform.isStartOfflinePush()) {
                ...
                try {
                    logger.info(
                            "[上级点播] 失败，推流设备未推流，channel: {}, app: {}, stream: {}",
                            gbStream.getGbId(),
                            gbStream.getApp(),
                            gbStream.getStream());
                    responseAck(
            ...
            // 发送redis消息以使设备上线
            logger.info(
                    "[ app={}, stream={} ]通道未推流，发送redis信息控制设备开始推流",
                    gbStream.getApp(),
                    gbStream.getStream());
            ...
            MessageForPushChannel messageForPushChannel =
                    MessageForPushChannel.getInstance(
                            ...
                            gbStream.getStream(),
                            gbStream.getGbId(),
                            gbStream.getPlatformId(),
                            platform.getName(),
                            null,
                            gbStream.getMediaServerId());
            ...
            dynamicTask.startDelay(
                    ...
                    () -> {
                        logger.info(
                                "[ app={}, stream={} ] 等待设备开始推流超时",
                                gbStream.getApp(),
                                gbStream.getStream());
                        try {
                            redisPushStreamResponseListener.removeEvent(
                                    gbStream.getApp(), gbStream.getStream());
                            mediaListManager.removedChannelOnlineEventLister(
                                    gbStream.getApp(), gbStream.getStream());
            ...
            // 添加在本机上线的通知
            mediaListManager.addChannelOnlineEventLister(
                    gbStream.getApp(),
                    gbStream.getStream(),
                    (app, stream, serverId) -> {
                        dynamicTask.stop(callIdHeader.getCallId());
                        ...
                        if (serverId.equals(userSetting.getServerId())) {
                            ...
                            SIPResponse response =
                                    sendStreamAck(
                                            mediaServerItem, request, sendRtpItem, platform, evt);
                            if (response != null) {
                                sendRtpItem.setToTag(response.getToTag());
                            }
                            redisCatchStorage.updateSendRTPSever(sendRtpItem);
            ...
            // 添加回复的拒绝或者错误的通知
            redisPushStreamResponseListener.addEvent(
                    gbStream.getApp(),
                    gbStream.getStream(),
                    response -> {
    ...
    private void otherWvpPushStream(
        ...
        // 发送redis消息
        redisGbPlayMsgListener.sendMsg(
                streamPushItem.getServerId(),
                streamPushItem.getMediaServerId(),
                streamPushItem.getApp(),
                streamPushItem.getStream(),
                addressStr,
                port,
                ssrc,
                requesterId,
                ...
                responseSendItemMsg -> {
                    ...
                    SIPResponse response =
                            sendStreamAck(
                                    responseSendItemMsg.getMediaServerItem(),
                                    request,
                                    sendRtpItem,
                                    platform,
                                    evt);
                ...
                (wvpResult) -> {
                    ...
                    if (wvpResult.getCode() == RedisGbPlayMsgListener.ERROR_CODE_OFFLINE) {
                        // 离线
                        // 查询是否在本机上线了
                        StreamPushItem currentStreamPushItem =
                                streamPushService.getPush(
                                        streamPushItem.getApp(), streamPushItem.getStream());
    ...
    public SIPResponse sendStreamAck(
            MediaServer mediaServerItem,
            SIPRequest request,
            SendRtpItem sendRtpItem,
            ParentPlatform platform,
    ...
    public void inviteFromDeviceHandle(SIPRequest request, String requesterId, String channelId) {
        ...
        if (device == null) {
            logger.warn(
                    "来自设备的Invite请求，无法从请求信息中确定所属设备，已忽略，requesterId： {}/{}", requesterId, channelId);
            try {
        ...
        AudioBroadcastCatch broadcastCatch =
                audioBroadcastManager.get(device.getDeviceId(), realChannelId);
        ...
        if (device != null) {
            ...
            try {
                ...
                Boolean streamReady =
                        mediaServerService.isStreamReady(
                                mediaServerItem,
                                broadcastCatch.getApp(),
                                broadcastCatch.getStream());
                if (streamReady) {
                    sendOk(
                            device,
                            sendRtpItem,
        ...
        try {
            ...
            content.append(
                    "o="
                            + config.getId()
                            + " "
                            + sdp.getOrigin().getSessionId()
                            + " "
                            + sdp.getOrigin().getSessionVersion()
                            + " IN IP4 "
            ...
            AudioBroadcastCatch audioBroadcastCatch =
                    audioBroadcastManager.get(device.getDeviceId(), sendRtpItem.getChannelId());
            ...
            if (!device.isBroadcastPushAfterAck()) {
                ...
                playService.startPushStream(
                        sendRtpItem, sipResponse, parentPlatform, request.getCallIdHeader());
            }
