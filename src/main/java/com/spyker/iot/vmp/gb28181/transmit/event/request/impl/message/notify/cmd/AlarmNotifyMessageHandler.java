...
/** 报警事件的处理，参考：9.4 */
@Component
public class AlarmNotifyMessageHandler extends SIPRequestProcessorParent
                    ...
                    () -> {
                        ...
                        while (!taskQueue.isEmpty()) {
                            try {
                                ...
                                deviceAlarm.setAlarmPriority(
                                        getText(sipMsgInfo.getRootElement(), "AlarmPriority"));
                                deviceAlarm.setAlarmMethod(
                                        getText(sipMsgInfo.getRootElement(), "AlarmMethod"));
                                String alarmTime =
                                        XmlUtil.getText(sipMsgInfo.getRootElement(), "AlarmTime");
                                if (alarmTime == null) {
                                    continue;
                                }
                                deviceAlarm.setAlarmTime(
                                        DateUtil.ISO8601Toyyyy_MM_dd_HH_mm_ss(alarmTime));
                                String alarmDescription =
                                        getText(sipMsgInfo.getRootElement(), "AlarmDescription");
                                ...
                                String longitude =
                                        getText(sipMsgInfo.getRootElement(), "Longitude");
                                ...
                                if (!ObjectUtils.isEmpty(deviceAlarm.getAlarmMethod())) {
                                    if (deviceAlarm
                                            .getAlarmMethod()
                                            .contains(DeviceAlarmMethod.GPS.getVal() + "")) {
                                        ...
                                        deviceChannel.setDeviceId(
                                                sipMsgInfo.getDevice().getDeviceId());
                                        ...
                                        mobilePosition.setLongitudeWgs84(
                                                deviceChannel.getLongitudeWgs84());
                                        mobilePosition.setLatitudeWgs84(
                                                deviceChannel.getLatitudeWgs84());
                                        mobilePosition.setLongitudeGcj02(
                                                deviceChannel.getLongitudeGcj02());
                                        mobilePosition.setLatitudeGcj02(
                                                deviceChannel.getLatitudeGcj02());
                                ...
                                if (!ObjectUtils.isEmpty(deviceAlarm.getDeviceId())) {
                                    if (deviceAlarm
                                            .getAlarmMethod()
                                            .contains(DeviceAlarmMethod.Video.getVal() + "")) {
                                        deviceAlarm.setAlarmType(
                                                getText(
                                                        sipMsgInfo.getRootElement().element("Info"),
                                                        "AlarmType"));
                                    }
                                }
                                logger.info("[收到报警通知]内容：{}", JSON.toJSONString(deviceAlarm));
                                // 作者自用判断，其他小伙伴需要此消息可以自行修改，但是不要提在pr里
                                if (DeviceAlarmMethod.Other.getVal()
                                        == Integer.parseInt(deviceAlarm.getAlarmMethod())) {
                                    // 发送给平台的报警信息。 发送redis通知
                                    logger.info(
                                            "[发送给平台的报警信息]内容：{}",
                                            JSONObject.toJSONString(deviceAlarm));
                                    AlarmChannelMessage alarmChannelMessage =
                                            new AlarmChannelMessage();
                                    if (deviceAlarm.getAlarmMethod() != null) {
                                        alarmChannelMessage.setAlarmSn(
                                                Integer.parseInt(deviceAlarm.getAlarmMethod()));
                                    }
                                    alarmChannelMessage.setAlarmDescription(
                                            deviceAlarm.getAlarmDescription());
                                    if (deviceAlarm.getAlarmType() != null) {
                                        alarmChannelMessage.setAlarmType(
                                                Integer.parseInt(deviceAlarm.getAlarmType()));
                                    }
                                ...
                                if (redisCatchStorage.deviceIsOnline(
                                        sipMsgInfo.getDevice().getDeviceId())) {
