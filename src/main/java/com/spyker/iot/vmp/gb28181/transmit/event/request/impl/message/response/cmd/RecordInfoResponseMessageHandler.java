...
import com.spyker.iot.vmp.utils.UJson;
 ...
 */
@Component
public class RecordInfoResponseMessageHandler extends SIPRequestProcessorParent
                ...
                () -> {
                    try {
                        ...
                        if (recordListElement == null || sumNum == 0) {
                            ...
                            Iterator<Element> recordListIterator =
                                    recordListElement.elementIterator();
                            if (recordListIterator != null) {
                                ...
                                while (recordListIterator.hasNext()) {
                                    ...
                                    record.setStartTime(
                                            DateUtil.ISO8601Toyyyy_MM_dd_HH_mm_ss(startTimeStr));

                                    String endTimeStr = getText(itemRecord, "EndTime");
                                    record.setEndTime(
                                            DateUtil.ISO8601Toyyyy_MM_dd_HH_mm_ss(endTimeStr));

                                    record.setSecrecy(
                                            itemRecord.element("Secrecy") == null
                                                    ? 0
                                                    : Integer.parseInt(
                                                            getText(itemRecord, "Secrecy")));
                                ...
                                // 获取任务结果数据
                                String resKey =
                                        VideoManagerConstants.REDIS_RECORD_INFO_RES_PRE
                                                + channelId
                                                + sn;
                                redisTemplate.opsForHash().putAll(resKey, map);
                                redisTemplate.expire(resKey, recordInfoTtl, TimeUnit.SECONDS);
                                String resCountKey =
                                        VideoManagerConstants.REDIS_RECORD_INFO_RES_COUNT_PRE
                                                + channelId
                                                + sn;
                                long incr =
                                        redisTemplate
                                                .opsForValue()
                                                .increment(resCountKey, map.size());
                                ...
                                // 已接收完成
                                List<RecordItem> resList =
                                        redisTemplate.opsForHash().entries(resKey).values().stream()
                                                .map(
                                                        e ->
                                                                UJson.readJson(
                                                                        e.toString(),
