...
public class MobilePositionSubscribeTask implements ISubscribeTask {
    ...
    public void run() {
        ...
        try {
            sipRequest =
                    sipCommander.mobilePositionSubscribe(
                            device,
                            request,
                            eventResult -> {
                                // 成功
                                ...
                                ToHeader toHeader =
                                        (ToHeader) event.getResponse().getHeader(ToHeader.NAME);
                                try {
                            ...
                            eventResult -> {
                                ...
                                dynamicTask.startDelay(
                                        taskKey, MobilePositionSubscribeTask.this, 2000);
                            });
    ...
    public void stop(CommonCallback<Boolean> callback) {
        /**
         * dialog 的各个状态 EARLY-> Early state状态-初始请求发送以后，收到了一个临时响应消息 CONFIRMED-> Confirmed
         * Dialog状态-已确认 COMPLETED-> Completed Dialog状态-已完成 TERMINATED-> Terminated Dialog状态-终止
         */
        if (dynamicTask.get(taskKey) != null) {
            dynamicTask.stop(taskKey);
        }
