package com.spyker.iot.vmp.gb28181.transmit.cmd.impl;
...
import com.spyker.iot.vmp.gb28181.SipLayer;
...
import com.spyker.iot.vmp.gb28181.transmit.cmd.SIPRequestHeaderPlarformProvider;
...
public class SIPCommanderFroPlatform implements ISIPCommanderForPlatform {
    ...
    @Autowired private SIPRequestHeaderPlarformProvider headerProviderPlatformProvider;
    ...
    @Autowired private SipLayer sipLayer;

    @Autowired private SIPSender sipSender;
    ...
    public void register(
            ParentPlatform parentPlatform,
            @Nullable SipTransactionInfo sipTransactionInfo,
            @Nullable WWWAuthenticateHeader www,
            SipSubscribe.Event errorEvent,
            SipSubscribe.Event okEvent,
            boolean isRegister)
        ...
        CallIdHeader callIdHeader =
                sipSender.getNewCallIdHeader(
                        parentPlatform.getDeviceIp(), parentPlatform.getTransport());
        ...
        if (www == null) {
            request =
                    headerProviderPlatformProvider.createRegisterRequest(
                            parentPlatform,
                            redisCatchStorage.getCSEQ(),
                            fromTag,
                            toTag,
                            callIdHeader,
                            isRegister ? parentPlatform.getExpires() : 0);
            // 将 callid 写入缓存， 等注册成功可以更新状态
            String callIdFromHeader = callIdHeader.getCallId();
            redisCatchStorage.updatePlatformRegisterInfo(
                    callIdFromHeader,
                    PlatformRegisterInfo.getInstance(parentPlatform.getServerGBId(), isRegister));
    ...
    @Override
    public void catalogQuery(
            List<DeviceChannel> channels, ParentPlatform parentPlatform, String sn, String fromTag)
            throws InvalidArgumentException, ParseException, SipException {
        if (parentPlatform == null) {
            return;
        }
    ...
    private String getCatalogXml(
            List<DeviceChannel> channels, String sn, ParentPlatform parentPlatform, int size) {
        String characterSet = parentPlatform.getCharacterSet();
            ...
            for (DeviceChannel channel : channels) {
                ...
                if (channel.getChannelId().length() <= 8) {
                    ...
                    if (channel.getChannelId().length() != 20) {
                        catalogXml.append("</Item>\r\n");
                        logger.warn(
                                "[编号长度异常] {} 长度错误，请使用20位长度的国标编号,当前长度：{}",
                                channel.getChannelId(),
                                channel.getChannelId().length());
                        catalogXml.append("</Item>\r\n");
                        continue;
                    }
                    switch (Integer.parseInt(channel.getChannelId().substring(10, 13))) {
                        case 200:
                            ...
                            //                            GitUtil gitUtil =
                            // SpringBeanFactory.getBean("gitUtil");
                            //                            String model = (gitUtil == null ||
                            // gitUtil.getBuildVersion() == null)?"1.0": gitUtil.getBuildVersion();
                        ...
                        case 215:
                            if (!ObjectUtils.isEmpty(channel.getParentId())) {
                                catalogXml.append(
                                        "<ParentID>" + channel.getParentId() + "</ParentID>\r\n");
                            }
                        ...
                        default:
                            ...
                            if (channel.getSecrecy() != null) {
                                catalogXml.append(
                                        "<Secrecy>" + channel.getSecrecy() + "</Secrecy>\r\n");
                            } else {
                                catalogXml.append("<Secrecy></Secrecy>\r\n");
                            }
                            catalogXml.append(
                                    "<RegisterWay>"
                                            + channel.getRegisterWay()
                                            + "</RegisterWay>\r\n");
                            ...
                            if (channel.getAddress() == null) {
                                ...
                                catalogXml.append(
                                        "<Address>" + channel.getAddress() + "</Address>\r\n");
                            }
                            if (!ObjectUtils.isEmpty(channel.getParentId())) {
                                catalogXml.append(
                                        "<ParentID>" + channel.getParentId() + "</ParentID>\r\n");
                            ...
                            if (!ObjectUtils.isEmpty(channel.getCertNum())) {
                                catalogXml.append(
                                        "<CertNum>" + channel.getCertNum() + "</CertNum>\r\n");
                            } else {
                            ...
                            if (!ObjectUtils.isEmpty(channel.getPassword())) {
                                catalogXml.append(
                                        "<Password>" + channel.getPassword() + "</Password>\r\n");
                            } else {
                                catalogXml.append("<Password></Password>\r\n");
                            }
                            if (!ObjectUtils.isEmpty(channel.getPtzType())) {
                                catalogXml.append(
                                        "<PTZType>" + channel.getPtzType() + "</PTZType>\r\n");
                            } else {
                                catalogXml.append("<PTZType></PTZType>\r\n");
                            }
                            catalogXml.append(
                                    "<Status>"
                                            + (channel.isStatus() ? "ON" : "OFF")
                                            + "</Status>\r\n");

                            catalogXml.append(
                                    "<Longitude>"
                                            + (channel.getLongitudeWgs84() != 0
                                                    ? channel.getLongitudeWgs84()
                                                    : channel.getLongitude())
                                            + "</Longitude>\r\n");
                            catalogXml.append(
                                    "<Latitude>"
                                            + (channel.getLatitudeWgs84() != 0
                                                    ? channel.getLatitudeWgs84()
                                                    : channel.getLatitude())
                                            + "</Latitude>\r\n");
                            break;
                    }
    ...
    private void sendCatalogResponse(
            List<DeviceChannel> channels,
            ParentPlatform parentPlatform,
            String sn,
            String fromTag,
            int index,
            boolean sendAfterResponse)
            throws SipException, InvalidArgumentException, ParseException {
        ...
        if (sendAfterResponse) {
            ...
            dynamicTask.startDelay(
                    ...
                    () -> {
                        ...
                        try {
                            sendCatalogResponse(
                                    channels, parentPlatform, sn, fromTag, indexNext, false);
            ...
            sipSender.transmitRequest(
                    parentPlatform.getDeviceIp(),
                    request,
                    eventResult -> {
                        logger.error(
                                "[目录推送失败] 国标级联 platform : {}, code: {}, msg: {}, 停止发送",
                                parentPlatform.getServerGBId(),
                    ...
                    eventResult -> {
                        ...
                        try {
                            sendCatalogResponse(
                                    channels, parentPlatform, sn, fromTag, indexNext, true);
                        } catch (SipException | InvalidArgumentException | ParseException e) {
                            logger.error("[命令发送失败] 国标级联 目录查询回复: {}", e.getMessage());
                        }
                    });
     ...
     */
    @Override
    public void deviceInfoResponse(
            ParentPlatform parentPlatform, Device device, String sn, String fromTag)
            throws SipException, InvalidArgumentException, ParseException {
        if (parentPlatform == null) {
            return;
        }
     ...
     */
    @Override
    public void deviceStatusResponse(
            ParentPlatform parentPlatform,
            String channelId,
            String sn,
            String fromTag,
    ...
    @Override
    public void sendNotifyMobilePosition(
            ParentPlatform parentPlatform, GPSMsgInfo gpsMsgInfo, SubscribeInfo subscribeInfo)
        ...
        sendNotify(
                ...
                eventResult -> {
                    logger.error(
                            "发送NOTIFY通知消息失败。错误：{} {}", eventResult.statusCode, eventResult.msg);
                },
                null);
    }

    @Override
    public void sendAlarmMessage(ParentPlatform parentPlatform, DeviceAlarm deviceAlarm)
            throws SipException, InvalidArgumentException, ParseException {
        if (parentPlatform == null) {
            return;
        }
    ...
    @Override
    public void sendNotifyForCatalogAddOrUpdate(
            String type,
            ParentPlatform parentPlatform,
            List<DeviceChannel> deviceChannels,
        ...
        String catalogXmlContent =
                getCatalogXmlContentForCatalogAddOrUpdate(
                        parentPlatform, channels, deviceChannels.size(), type, subscribeInfo);
    ...
    private void sendNotify(
        ...
        MessageFactoryImpl messageFactory =
                (MessageFactoryImpl) SipFactory.getInstance().createMessageFactory();
        String characterSet = parentPlatform.getCharacterSet();
        // 设置编码， 防止中文乱码
        messageFactory.setDefaultContentEncodingCharset(characterSet);

        SIPRequest notifyRequest =
                headerProviderPlatformProvider.createNotifyRequest(
                        parentPlatform, catalogXmlContent, subscribeInfo);

        sipSender.transmitRequest(parentPlatform.getDeviceIp(), notifyRequest, errorEvent, okEvent);
    }

    private String getCatalogXmlContentForCatalogAddOrUpdate(
            ParentPlatform parentPlatform,
            List<DeviceChannel> channels,
            int sumNum,
            String type,
        ...
        if (!channels.isEmpty()) {
            for (DeviceChannel channel : channels) {
                ...
                if (channel.getChannelId().length() == 20
                        && Integer.parseInt(channel.getChannelId().substring(10, 13)) == 216) {
                    // 虚拟组织增加BusinessGroupID字段
                    catalogXml.append(
                            "<BusinessGroupID>" + channel.getParentId() + "</BusinessGroupID>\r\n");
                }
                ...
                if (channel.getParental() == 0) {
                    ...
                    if (channel.getChannelType() != 2) { // 业务分组/虚拟组织/行政区划 不设置以下属性
                        catalogXml
                                .append("<Model>" + channel.getModel() + "</Model>\r\n")
    ...
    public void sendNotifyForCatalogOther(
        ...
        Integer finalIndex = index;
        String catalogXmlContent =
                getCatalogXmlContentForCatalogOther(parentPlatform, channels, type);
        sendNotify(
                parentPlatform,
                catalogXmlContent,
                subscribeInfo,
    ...
    private String getCatalogXmlContentForCatalogOther(
            ParentPlatform parentPlatform, List<DeviceChannel> channels, String type) {
                ...
                for (RecordItem recordItem : recordInfo.getRecordList()) {
                    ...
                    if (deviceChannel != null) {
                        recordXml
                                ...
                                .append(
                                        "<StartTime>"
                                                + DateUtil.yyyy_MM_dd_HH_mm_ssToISO8601(
                                                        recordItem.getStartTime())
                                                + "</StartTime>\r\n")
                                .append(
                                        "<EndTime>"
                                                + DateUtil.yyyy_MM_dd_HH_mm_ssToISO8601(
                                                        recordItem.getEndTime())
                                                + "</EndTime>\r\n")
                        ...
                        if (!ObjectUtils.isEmpty(recordItem.getFileSize())) {
                            recordXml.append(
                                    "<FileSize>" + recordItem.getFileSize() + "</FileSize>\r\n");
                        }
                        if (!ObjectUtils.isEmpty(recordItem.getFilePath())) {
                            recordXml.append(
                                    "<FilePath>" + recordItem.getFilePath() + "</FilePath>\r\n");
                        }
                    }
                    recordXml.append("</Item>\r\n");
                }
    ...
    @Override
    public void sendMediaStatusNotify(ParentPlatform parentPlatform, SendRtpItem sendRtpItem)
            throws SipException, InvalidArgumentException, ParseException {
        ...
        SIPRequest messageRequest =
                (SIPRequest)
                        headerProviderPlatformProvider.createMessageRequest(
                                parentPlatform, mediaStatusXml, sendRtpItem);
    ...
    public void streamByeCmd(ParentPlatform platform, String callId)
        ...
        SendRtpItem sendRtpItem =
                redisCatchStorage.querySendRTPServer(platform.getServerGBId(), null, null, callId);
        if (sendRtpItem != null) {
    ...
    @Override
    public synchronized void streamByeCmd(ParentPlatform platform, SendRtpItem sendRtpItem)
            throws SipException, InvalidArgumentException, ParseException {
        if (sendRtpItem == null) {
        ...
        SIPRequest byeRequest =
                headerProviderPlatformProvider.createByeRequest(platform, sendRtpItem);
        if (byeRequest == null) {
    ...
    public void streamByeCmd(
        ...
        SsrcTransaction ssrcTransaction =
                streamSession.getSsrcTransaction(
                        platform.getServerGBId(), channelId, callId, stream);
        if (ssrcTransaction == null) {
            throw new SsrcTransactionNotFoundException(
                    platform.getServerGBId(), channelId, callId, stream);
        }

        mediaServerService.releaseSsrc(
                ssrcTransaction.getMediaServerId(), ssrcTransaction.getSsrc());
        mediaServerService.closeRTPServer(
                ssrcTransaction.getMediaServerId(), ssrcTransaction.getStream());
        ...
        Request byteRequest =
                headerProviderPlatformProvider.createByteRequest(
                        platform, channelId, ssrcTransaction.getSipTransactionInfo());
        sipSender.transmitRequest(
                sipLayer.getLocalIp(platform.getDeviceIp()), byteRequest, null, okEvent);
    }
    ...
    @Override
    public void broadcastInviteCmd(
            ParentPlatform platform,
            String channelId,
            MediaServer mediaServerItem,
            SSRCInfo ssrcInfo,
            HookSubscribe.Event event,
            SipSubscribe.Event okEvent,
            SipSubscribe.Event errorEvent)
        ...
        logger.info(
                ...
                ssrcInfo.getPort());
        Hook hook =
                Hook.getInstance(HookType.on_media_arrival, "rtp", stream, mediaServerItem.getId());
        ...
        CallIdHeader callIdHeader =
                sipSender.getNewCallIdHeader(
                        sipLayer.getLocalIp(platform.getDeviceIp()), platform.getTransport());

        Request request =
                headerProviderPlatformProvider.createInviteRequest(
                        platform,
                        channelId,
                        content.toString(),
                        SipUtils.getNewViaTag(),
                        ...
                        callIdHeader);
        sipSender.transmitRequest(
                sipLayer.getLocalIp(platform.getDeviceIp()),
                request,
                (e -> {
