...
import com.spyker.iot.vmp.service.bean.RequestPushStreamMsg;
 ...
 */
@Component
public class AckRequestProcessor extends SIPRequestProcessorParent
    ...
    public void process(RequestEvent evt) {
        ...
        String fromUserId =
                ((SipURI)
                                ((HeaderAddress) evt.getRequest().getHeader(FromHeader.NAME))
                                        .getAddress()
                                        .getURI())
                        .getUser();
        String toUserId =
                ((SipURI)
                                ((HeaderAddress) evt.getRequest().getHeader(ToHeader.NAME))
                                        .getAddress()
                                        .getURI())
                        .getUser();
        ...
        if (parentPlatform != null) {
            if (!userSetting.getServerId().equals(sendRtpItem.getServerId())) {
                ...
                redisGbPlayMsgListener.sendMsgForStartSendRtpStream(
                        ...
                        () -> {
                            playService.startSendRtpStreamFailHand(
                                    sendRtpItem, parentPlatform, callIdHeader);
                        });
            } else {
                try {
                    if (sendRtpItem.isTcpActive()) {
                        mediaServerService.startSendRtpPassive(
                                mediaInfo, parentPlatform, sendRtpItem, null);
                    } else {
