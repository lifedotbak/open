...
import com.spyker.iot.vmp.gb28181.bean.RecordItem;
    ...
    private void timerTask() {
        ...
        for (String key : keys) {
            ...
            if (recordInfo.getLastTime().isBefore(instantBefore5S)) {
                // 处理录像数据， 返回给前端
                String msgKey =
                        DeferredResultHolder.CALLBACK_CMD_RECORDINFO
                                + recordInfo.getDeviceId()
                                + recordInfo.getSn();
                ...
                recordEndEventListener.delEndEventHandler(
                        recordInfo.getDeviceId(), recordInfo.getChannelId());
                data.remove(key);
            }
        }
    }
