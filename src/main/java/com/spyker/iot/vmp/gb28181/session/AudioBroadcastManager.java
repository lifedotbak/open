...
public class AudioBroadcastManager {
    ...
    private static final Logger logger = LoggerFactory.getLogger(AudioBroadcastManager.class);
    ...
    public void update(AudioBroadcastCatch audioBroadcastCatch) {
        if (SipUtils.isFrontEnd(audioBroadcastCatch.getDeviceId())) {
            ...
            data.put(
                    audioBroadcastCatch.getDeviceId() + audioBroadcastCatch.getChannelId(),
                    audioBroadcastCatch);
        }
    }
    ...
    public AudioBroadcastCatch get(String deviceId, String channelId) {
        ...
        if (audioBroadcastCatch == null) {
            Stream<AudioBroadcastCatch> allAudioBroadcastCatchStreamForDevice =
                    data.values().stream()
                            .filter(
                                    audioBroadcastCatchItem ->
                                            Objects.equals(
                                                    audioBroadcastCatchItem.getDeviceId(),
                                                    deviceId));
            List<AudioBroadcastCatch> audioBroadcastCatchList =
                    allAudioBroadcastCatchStreamForDevice.collect(Collectors.toList());
