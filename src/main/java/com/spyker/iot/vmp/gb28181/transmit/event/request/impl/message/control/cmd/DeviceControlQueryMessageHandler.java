package com.spyker.iot.vmp.gb28181.transmit.event.request.impl.message.control.cmd;

import static com.spyker.iot.vmp.gb28181.utils.XmlUtil.getText;
import static com.spyker.iot.vmp.gb28181.utils.XmlUtil.loadElement;

import com.spyker.iot.vmp.common.enums.DeviceControlType;
import com.spyker.iot.vmp.gb28181.bean.Device;
import com.spyker.iot.vmp.gb28181.bean.DragZoomRequest;
import com.spyker.iot.vmp.gb28181.bean.HomePositionRequest;
...
import com.spyker.iot.vmp.gb28181.transmit.event.request.impl.message.control.ControlMessageHandler;
...
@Component
public class DeviceControlQueryMessageHandler extends SIPRequestProcessorParent
    ...
    public void handForPlatform(
        ...
        if (!ObjectUtils.isEmpty(deviceControlType)
            ...
            switch (deviceControlType) {
                ...
                case GUARD:
                    handleGuardCmd(
                            deviceForPlatform, rootElement, request, DeviceControlType.GUARD);
                    break;
                case RECORD:
                    handleRecordCmd(
                            deviceForPlatform,
                ...
                case DRAG_ZOOM_IN:
                    handleDragZoom(
                            ...
                            rootElement,
                            request,
                            DeviceControlType.DRAG_ZOOM_IN);
                    break;
                case DRAG_ZOOM_OUT:
                    handleDragZoom(
                            deviceForPlatform,
                            ...
                            rootElement,
                            request,
                            DeviceControlType.DRAG_ZOOM_OUT);
                    break;
                case HOME_POSITION:
                    handleHomePositionCmd(
                            deviceForPlatform,
                            channelId,
                            rootElement,
                            request,
                            DeviceControlType.HOME_POSITION);
                    break;
                default:
                    break;
            }
     ...
     * @param type 消息类型
     */
    private void handleHomePositionCmd(
            Device device,
            String channelId,
            Element rootElement,
            SIPRequest request,
        ...
        try {
            ...
            cmder.homePositionCmd(
                    device,
                    channelId,
                    !"0".equals(info.getEnabled()),
                    Integer.parseInt(info.getResetTime()),
                    Integer.parseInt(info.getPresetIndex()),
                    errorResult -> onError(request, errorResult),
                    okResult -> onOk(request, okResult));
        } catch (Exception e) {
     ...
     * @param type 消息类型
     */
    private void handleGuardCmd(
            Device device, Element rootElement, SIPRequest request, DeviceControlType type) {
        // 获取整个消息主体，我们只需要修改请求头即可
        String cmdString = getText(rootElement, type.getVal());
        try {
            cmder.guardCmd(
