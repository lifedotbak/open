package com.spyker.iot.vmp.gb28181;
...
public class SipLayer implements CommandLineRunner {
    ...
    @Autowired private ISIPProcessorObserver sipProcessorObserver;
    ...
    private void addListeningPoint(String monitorIp, int port) {
                            ...
                            SipFactory.getInstance()
                                    .createSipStack(
                                            DefaultProperties.getProperties(
                                                    "GB28181_SIP", userSetting.getSipLog()));
        ...
        try {
            ListeningPoint tcpListeningPoint =
                    sipStack.createListeningPoint(monitorIp, port, "TCP");
            SipProviderImpl tcpSipProvider =
                    (SipProviderImpl) sipStack.createSipProvider(tcpListeningPoint);
        ...
        try {
            ListeningPoint udpListeningPoint =
                    sipStack.createListeningPoint(monitorIp, port, "UDP");

            SipProviderImpl udpSipProvider =
                    (SipProviderImpl) sipStack.createSipProvider(udpListeningPoint);
