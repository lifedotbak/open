    ...
    public void handForDevice(RequestEvent evt, Device device, Element rootElement) {
        ...
        if ("121".equals(NotifyType)) {
            ...
            SsrcTransaction ssrcTransaction =
                    streamSession.getSsrcTransaction(null, null, callIdHeader.getCallId(), null);
            if (ssrcTransaction != null) {
                logger.info(
                        "[录像流]推送完毕，关流通知， device: {}, channelId: {}",
                ...
                try {
                    cmder.streamByeCmd(
                            device, ssrcTransaction.getChannelId(), null, callIdHeader.getCallId());
                ...
                SendRtpItem sendRtpItem =
                        redisCatchStorage.querySendRTPServer(
                                null, ssrcTransaction.getChannelId(), null, null);
                if (sendRtpItem != null) {
                    ParentPlatform parentPlatform =
                            storage.queryParentPlatByServerGBId(sendRtpItem.getPlatformId());
                    if (parentPlatform == null) {
                        logger.warn(
                                "[级联消息发送]：发送MediaStatus发现上级平台{}不存在", sendRtpItem.getPlatformId());
                        return;
                    }
                    try {
