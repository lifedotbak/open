...
public class SendRtpPortManager {
    ...
    private static final Logger logger = LoggerFactory.getLogger(SendRtpPortManager.class);
    ...
    public synchronized int getNextPort(MediaServer mediaServer) {
        ...
        if (sendRtpPortRange != null) {
            ...
            if (portArray.length != 2
                    || !NumberUtils.isParsable(portArray[0])
                ...
                if (Integer.parseInt(portArray[1]) - Integer.parseInt(portArray[0]) < 1) {
                    logger.warn(
                            "{}发送端口配置错误,结束端口至少比开始端口大一，自动使用50000-60000作为端口范围", mediaServer.getId());
                    startPort = 50000;
                    endPort = 60000;
                } else {
        ...
        //            return getPort(current, startPort, endPort, checkPort->
        // !sendRtpItemMap.containsKey(checkPort));
        //        });
        return getSendPort(startPort, endPort, sendIndexKey, sendRtpItemMap);
    }

    private synchronized int getSendPort(
            int startPort,
            int endPort,
            String sendIndexKey,
            Map<Integer, SendRtpItem> sendRtpItemMap) {
        // TODO 这里改为只取偶数端口
        RedisAtomicInteger redisAtomicInteger =
                new RedisAtomicInteger(sendIndexKey, redisTemplate.getConnectionFactory());
