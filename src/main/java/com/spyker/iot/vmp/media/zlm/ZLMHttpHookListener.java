...
import com.spyker.iot.vmp.media.zlm.dto.hook.*;
import com.spyker.iot.vmp.media.zlm.event.HookZlmServerKeepaliveEvent;
import com.spyker.iot.vmp.media.zlm.event.HookZlmServerStartEvent;
...
public class ZLMHttpHookListener {
    ...
    public HookResult onPlay(@RequestBody OnPlayHookParam param) {
        ...
        boolean authenticateResult =
                mediaService.authenticatePlay(
                        param.getApp(), param.getStream(), paramMap.get("callId"));
    ...
    public HookResultForOnPublish onPublish(@RequestBody OnPublishHookParam param) {
        ...
        ResultForOnPublish resultForOnPublish =
                mediaService.authenticatePublish(
                        mediaServer, param.getApp(), param.getStream(), param.getParams());
        if (resultForOnPublish != null) {
            HookResultForOnPublish successResult =
                    HookResultForOnPublish.getInstance(resultForOnPublish);
            logger.info(
                    "[ZLM HOOK]推流鉴权 响应：{}->{}->>>>{}",
    ...
    public HookResult onStreamChanged(@RequestBody OnStreamChangedHookParam param) {
        ...
        if (param.isRegist()) {
            logger.info(
                    ...
                    param.getApp(),
                    param.getStream());
            MediaArrivalEvent mediaArrivalEvent =
                    MediaArrivalEvent.getInstance(this, param, mediaServer);
            ...
            logger.info(
                    ...
                    param.getApp(),
                    param.getStream());
            MediaDepartureEvent mediaDepartureEvent =
    ...
    public HookResult onStreamNotFound(@RequestBody OnStreamNotFoundHookParam param) {
        ...
        MediaNotFoundEvent mediaNotFoundEvent =
                MediaNotFoundEvent.getInstance(this, param, mediaServer);
    ...
    public HookResult onServerStarted(
        ...
        try {
            ...
            MediaServer mediaServerItem =
                    mediaServerService.getOne(zlmServerConfig.getMediaServerId());
            if (mediaServerItem != null) {
                event.setMediaServerItem(mediaServerItem);
                applicationEventPublisher.publishEvent(event);
            }
    ...
    public HookResult onSendRtpStopped(
            HttpServletRequest request, @RequestBody OnSendRtpStoppedHookParam param) {
    ...
    public HookResult onRecordMp4(
            HttpServletRequest request, @RequestBody OnRecordMp4HookParam param) {
        logger.info("[ZLM HOOK] 录像完成事件：{}->{}", param.getMediaServerId(), param.getFile_path());

        try {
            MediaServer mediaServerItem = mediaServerService.getOne(param.getMediaServerId());
            if (mediaServerItem != null) {
                MediaRecordMp4Event event =
                        MediaRecordMp4Event.getInstance(this, param, mediaServerItem);
                event.setMediaServer(mediaServerItem);
                applicationEventPublisher.publishEvent(event);
            }
        } catch (Exception e) {
