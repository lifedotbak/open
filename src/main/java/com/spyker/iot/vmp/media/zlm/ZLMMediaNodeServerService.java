...
import com.spyker.iot.vmp.media.zlm.dto.ZLMServerConfig;
...
public class ZLMMediaNodeServerService implements IMediaNodeServerService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ZLMMediaNodeServerService.class);

    @Autowired private ZLMRESTfulUtils zlmresTfulUtils;

    @Autowired private ZLMServerFactory zlmServerFactory;

    @Value("${sip.ip}")
    private String sipIp;

    @Override
    public int createRTPServer(
            ...
            Boolean reUsePort,
            Integer tcpMode) {
        return zlmServerFactory.createRTPServer(
                mediaServer, streamId, ssrc, port, onlyAuto, reUsePort, tcpMode);
    }
    ...
    public boolean checkNodeId(MediaServer mediaServer) {
        ...
        if (responseJSON != null) {
            ...
            if (data != null && !data.isEmpty()) {
                ZLMServerConfig zlmServerConfig =
                        JSON.parseObject(JSON.toJSONString(data.get(0)), ZLMServerConfig.class);
    ...
    public boolean deleteRecordDirectory(
        ...
        logger.info(
                "[zlm-deleteRecordDirectory] 删除磁盘文件, server: {} {}:{}->{}/{}",
                mediaServer.getId(),
                app,
                stream,
                date,
        ...
        if (jsonObject.getInteger("code") == 0) {
            ...
            logger.info(
                    "[zlm-deleteRecordDirectory] 删除磁盘文件错误, server: {} {}:{}->{}/{}, 结果： {}",
                    mediaServer.getId(),
                    app,
                    stream,
                    date,
            ...
            if (mediaList.getInteger("code") == 0) {
                ...
                StreamInfo streamInfo =
                        getStreamInfoByAppAndStream(
                                mediaServer, app, stream, mediaInfo, callId, true);
                if (streamInfo != null) {
    ...
    public StreamInfo getStreamInfoByAppAndStream(
        ...
        streamInfoResult.setFlv(
                addr, mediaServer.getHttpPort(), mediaServer.getHttpSSlPort(), flvFile);
    ...
    public Boolean connectRtpServer(
        ...
        JSONObject jsonObject =
                zlmresTfulUtils.connectRtpServer(mediaServer, address, port, stream);
    ...
    public WVPResult<String> addFFmpegSource(
            ...
            String ffmpegCmdKey) {
        JSONObject jsonObject =
                zlmresTfulUtils.addFFmpegSource(
                        mediaServer,
                        srcUrl,
                        dstUrl,
                        timeoutMs,
    ...
    public WVPResult<String> addStreamProxy(
            ...
            String rtpType) {
        JSONObject jsonObject =
                zlmresTfulUtils.addStreamProxy(
    ...
    public Map<String, String> getFFmpegCMDs(MediaServer mediaServer) {
        ...
        if (mediaServerConfigResuly != null
                && mediaServerConfigResuly.getInteger("code") == 0
                && mediaServerConfigResuly.getJSONArray("data").size() > 0) {
            JSONObject mediaServerConfig =
                    mediaServerConfigResuly.getJSONArray("data").getJSONObject(0);
    ...
    public void startSendRtpPassive(
            MediaServer mediaServer, SendRtpItem sendRtpItem, Integer timeout) {
