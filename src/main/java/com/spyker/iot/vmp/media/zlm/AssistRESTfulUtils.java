...
import com.spyker.iot.vmp.utils.SSLSocketClientUtil;
...
public class AssistRESTfulUtils {
    ...
    private static final Logger logger = LoggerFactory.getLogger(AssistRESTfulUtils.class);
    ...
    private OkHttpClient getClient(Integer readTimeOut) {
        if (client == null) {
            ...
            httpClientBuilder.sslSocketFactory(
                    SSLSocketClientUtil.getSocketFactory(manager), manager);
    ...
    public JSONObject sendGet(
        ...
        if (callback == null) {
            try {
                ...
                logger.error(
                        String.format(
                                "连接Assist失败: %s, %s", e.getCause().getMessage(), e.getMessage()));
    ...
    public JSONObject sendPost(
        ...
        RequestBody requestBodyJson =
                RequestBody.create(
                        MediaType.parse("application/json; charset=utf-8"), param.toString());
    ...
    public JSONObject queryTaskList(
            MediaServer mediaServerItem,
            String app,
            String stream,
            String callId,
        ...
        String urlStr =
                String.format(
                        "%s://%s:%s/api/record/file/download/task/list",
