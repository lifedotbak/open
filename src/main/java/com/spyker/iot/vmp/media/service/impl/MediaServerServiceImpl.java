package com.spyker.iot.vmp.media.service.impl;
...
import com.spyker.iot.vmp.conf.MediaConfig;
...
import com.spyker.iot.vmp.media.event.mediaServer.MediaServerChangeEvent;
import com.spyker.iot.vmp.media.event.mediaServer.MediaServerDeleteEvent;
import com.spyker.iot.vmp.media.service.IMediaNodeServerService;
...
import com.spyker.iot.vmp.media.zlm.dto.hook.OriginType;
...
import com.spyker.iot.vmp.storager.dao.MediaServerMapper;
...
public class MediaServerServiceImpl implements IMediaServerService {
    ...
    @Autowired private Map<String, IMediaNodeServerService> nodeServerServiceMap;

    @Autowired private ApplicationEventPublisher applicationEventPublisher;

    @Autowired private MediaConfig mediaConfig;
    ...
    public void onApplicationEvent(MediaDepartureEvent event) {
        if ("rtsp".equals(event.getSchema())) {
            ...
            MediaInfo mediaInfo =
                    redisCatchStorage.getStreamInfo(
                            event.getApp(), event.getStream(), event.getMediaServer().getId());
            if (mediaInfo == null) {
                return;
            }
            String type = OriginType.values()[mediaInfo.getOriginType()].getType();
            redisCatchStorage.removeStream(
                    mediaInfo.getMediaServer().getId(), type, event.getApp(), event.getStream());
        }
    }
    ...
    public void updateVmServer(List<MediaServer> mediaServerList) {
        ...
        for (MediaServer mediaServer : mediaServerList) {
            ...
            String key =
                    VideoManagerConstants.MEDIA_SERVER_PREFIX
    ...
    public SSRCInfo openRTPServer(
        ...
        if (ssrcCheck && tcpMode > 0) {
            ...
            logger.warn(
                    "[openRTPServer] 平台对接时下级可能自定义ssrc，但是tcp模式zlm收流目前无法更新ssrc，可能收流超时，此时请使用udp收流或者关闭ssrc校验");
        }
        int rtpServerPort;
        if (mediaServer.isRtpEnable()) {
            IMediaNodeServerService mediaNodeServerService =
                    nodeServerServiceMap.get(mediaServer.getType());
            if (mediaNodeServerService == null) {
                logger.info(
                        "[openRTPServer] 失败, mediaServer的类型： {}，未找到对应的实现类", mediaServer.getType());
                return null;
            }
            rtpServerPort =
                    mediaNodeServerService.createRTPServer(
                            mediaServer,
                            streamId,
                            ssrcCheck ? Long.parseLong(ssrc) : 0,
                            port,
                            onlyAuto,
                            disableAudio,
                            reUsePort,
    ...
    public void closeRTPServer(
            MediaServer mediaServer, String streamId, CommonCallback<Boolean> callback) {
        if (mediaServer == null) {
            callback.run(false);
            return;
        }
    ...
    public Boolean updateRtpServerSSRC(MediaServer mediaServer, String streamId, String ssrc) {
        ...
        if (mediaNodeServerService == null) {
            logger.info(
                    "[updateRtpServerSSRC] 失败, mediaServer的类型： {}，未找到对应的实现类",
                    mediaServer.getType());
            return false;
        }
    ...
    public void update(MediaServer mediaSerItem) {
        ...
        if (mediaServerInRedis == null
                || !ssrcFactory.hasMediaServerSSRC(mediaServerInDataBase.getId())) {
    ...
    public List<MediaServer> getAllOnlineList() {
                ...
                RedisUtil.scan(
                        ...
                        String.format(
                                ...
                                VideoManagerConstants.MEDIA_SERVER_PREFIX
                                        ...
                                        + ":"));
        String onlineKey =
                VideoManagerConstants.ONLINE_MEDIA_SERVERS_PREFIX + userSetting.getServerId();
        for (Object mediaServerKey : mediaServerKeys) {
            String key = (String) mediaServerKey;
            MediaServer mediaServer =
                    JsonUtil.redisJsonToObject(redisTemplate, key, MediaServer.class);
        ...
        result.sort(
                (serverItem1, serverItem2) -> {
                    int sortResult = 0;
                    LocalDateTime localDateTime1 =
                            LocalDateTime.parse(serverItem1.getCreateTime(), DateUtil.formatter);
                    LocalDateTime localDateTime2 =
                            LocalDateTime.parse(serverItem2.getCreateTime(), DateUtil.formatter);
    ...
    public void add(MediaServer mediaServer) {
        ...
        if (mediaServer.getHookAliveInterval() == null
                || mediaServer.getHookAliveInterval() == 0F) {
        ...
        if (mediaServerMapper.queryOne(mediaServer.getId()) != null) {
            ...
            throw new ControllerException(
                    ErrorCode.ERROR100.getCode(),
                    "保存失败，媒体服务ID [ " + mediaServer.getId() + " ] 已存在，请修改媒体服务器配置");
        }
        IMediaNodeServerService mediaNodeServerService =
                nodeServerServiceMap.get(mediaServer.getType());
        if (mediaNodeServerService == null) {
    ...
    public MediaServer checkMediaServer(String ip, int port, String secret, String type) {
        ...
        if (mediaServer != null) {
            if (mediaServerMapper.queryOne(mediaServer.getId()) != null) {
                throw new ControllerException(
                        ErrorCode.ERROR100.getCode(),
                        "媒体服务ID [" + mediaServer.getId() + " ] 已存在，请修改媒体服务器配置");
            }
        }
        return mediaServer;
    }
    ...
    public void delete(String id) {
        ...
        redisTemplate
                .opsForZSet()
                .remove(
                        VideoManagerConstants.ONLINE_MEDIA_SERVERS_PREFIX
                                + userSetting.getServerId(),
                        id);
        String key =
                VideoManagerConstants.MEDIA_SERVER_PREFIX + userSetting.getServerId() + ":" + id;
        redisTemplate.delete(key);
        // 发送节点移除通知
    ...
    public boolean stopSendRtp(MediaServer mediaInfo, String app, String stream, String ssrc) {
        IMediaNodeServerService mediaNodeServerService =
                nodeServerServiceMap.get(mediaInfo.getType());
        if (mediaNodeServerService == null) {
    ...
    @Override
    public boolean deleteRecordDirectory(
            MediaServer mediaServer, String app, String stream, String date, String fileName) {
        IMediaNodeServerService mediaNodeServerService =
                nodeServerServiceMap.get(mediaServer.getType());
        if (mediaNodeServerService == null) {
        ...
        return mediaNodeServerService.deleteRecordDirectory(
                mediaServer, app, stream, date, fileName);
    }

    @Override
    public List<StreamInfo> getMediaList(
            MediaServer mediaServer, String app, String stream, String callId) {
        IMediaNodeServerService mediaNodeServerService =
                nodeServerServiceMap.get(mediaServer.getType());
        if (mediaNodeServerService == null) {
    ...
    @Override
    public Boolean connectRtpServer(
            MediaServer mediaServer, String address, int port, String stream) {
        IMediaNodeServerService mediaNodeServerService =
                nodeServerServiceMap.get(mediaServer.getType());
        if (mediaNodeServerService == null) {
            logger.info(
                    "[connectRtpServer] 失败, mediaServer的类型： {}，未找到对应的实现类", mediaServer.getType());
            return false;
        }
    ...
    public void getSnap(
        ...
        mediaNodeServerService.getSnap(
                mediaServer, streamUrl, timeoutSec, expireSec, path, fileName);
    }
    ...
    @Override
    public WVPResult<String> addFFmpegSource(
            MediaServer mediaServer,
            String srcUrl,
            String dstUrl,
            int timeoutMs,
        ...
        if (mediaNodeServerService == null) {
            logger.info(
                    "[addFFmpegSource] 失败, mediaServer的类型： {}，未找到对应的实现类", mediaServer.getType());
            return WVPResult.fail(ErrorCode.ERROR400);
        }
        return mediaNodeServerService.addFFmpegSource(
                mediaServer, srcUrl, dstUrl, timeoutMs, enableAudio, enableMp4, ffmpegCmdKey);
    }

    @Override
    public WVPResult<String> addStreamProxy(
            MediaServer mediaServer,
            String app,
            String stream,
            String url,
        ...
        return mediaNodeServerService.addStreamProxy(
                mediaServer, app, stream, url, enableAudio, enableMp4, rtpType);
    }
    ...
    public Boolean delFFmpegSource(MediaServer mediaServer, String streamKey) {
        ...
        if (mediaNodeServerService == null) {
            logger.info(
                    "[delFFmpegSource] 失败, mediaServer的类型： {}，未找到对应的实现类", mediaServer.getType());
            return false;
        }
    ...
    public Boolean delStreamProxy(MediaServer mediaServerItem, String streamKey) {
        IMediaNodeServerService mediaNodeServerService =
                nodeServerServiceMap.get(mediaServerItem.getType());
        if (mediaNodeServerService == null) {
            logger.info(
                    "[delStreamProxy] 失败, mediaServer的类型： {}，未找到对应的实现类", mediaServerItem.getType());
            return false;
        }
    ...
    @Override
    public StreamInfo getStreamInfoByAppAndStream(
            MediaServer mediaServerItem,
            String app,
            String stream,
            MediaInfo mediaInfo,
            String callId) {
        return getStreamInfoByAppAndStream(
                mediaServerItem, app, stream, mediaInfo, null, callId, true);
    }

    @Override
    public StreamInfo getStreamInfoByAppAndStreamWithCheck(
    ...
    public StreamInfo getStreamInfoByAppAndStream(
        ...
        streamInfoResult.setRtmp(
                addr,
                mediaServer.getRtmpPort(),
                mediaServer.getRtmpSSlPort(),
                app,
                stream,
                callIdParam);
        streamInfoResult.setRtsp(
                addr,
                mediaServer.getRtspPort(),
                mediaServer.getRtspSSLPort(),
                app,
                stream,
                callIdParam);
        ...
        if ("abl".equals(mediaServer.getType())) {
            ...
            streamInfoResult.setFlv(
                    addr, mediaServer.getFlvPort(), mediaServer.getFlvSSLPort(), flvFile);
            streamInfoResult.setWsFlv(
                    addr, mediaServer.getWsFlvPort(), mediaServer.getWsFlvSSLPort(), flvFile);
        } else {
    ...
    public void startSendRtpPassive(
        ...
        if (mediaNodeServerService == null) {
            logger.info(
                    "[startSendRtpPassive] 失败, mediaServer的类型： {}，未找到对应的实现类",
                    mediaServer.getType());
    ...
    public void startSendRtp(
            MediaServer mediaServer, ParentPlatform platform, SendRtpItem sendRtpItem) {
        IMediaNodeServerService mediaNodeServerService =
                nodeServerServiceMap.get(mediaServer.getType());
        if (mediaNodeServerService == null) {
            logger.info(
                    "[startSendRtpStream] 失败, mediaServer的类型： {}，未找到对应的实现类", mediaServer.getType());
            throw new ControllerException(ErrorCode.ERROR100.getCode(), "未找到mediaServer对应的实现类");
        }
        logger.info(
                "[开始推流] rtp/{}, 目标={}:{}，SSRC={}, RTCP={}",
    ...
    @Override
    public SendRtpItem createSendRtpItem(
            MediaServer mediaServer,
            String ip,
            int port,
            String ssrc,
