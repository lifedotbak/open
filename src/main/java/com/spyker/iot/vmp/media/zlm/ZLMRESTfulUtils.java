...
public class ZLMRESTfulUtils {
        ...
        try {
            ...
            if (response.isSuccessful()) {
                if (targetPath != null) {
                    ...
                    logger.error(
                            String.format(
                                    "[ %s ]请求失败: %s %s", url, response.code(), response.message()));
                }
            } else {
                logger.error(
                        String.format(
            ...
            logger.error(
                    String.format("连接ZLM失败: %s, %s", e.getCause().getMessage(), e.getMessage()));
    ...
    public JSONObject isMediaOnline(
            MediaServer mediaServerItem, String app, String stream, String schema) {
    ...
    public JSONObject getMediaInfo(
            MediaServer mediaServerItem, String app, String schema, String stream) {
        Map<String, Object> param = new HashMap<>();
        param.put("app", app);
        param.put("schema", schema);
        param.put("stream", stream);
    ...
    public JSONObject addFFmpegSource(
            MediaServer mediaServerItem,
            String src_url,
            String dst_url,
            Integer timeout_ms,
    ...
    public void closeRtpServer(
            MediaServer mediaServerItem, Map<String, Object> param, RequestCallback callback) {
    ...
    public JSONObject startSendRtpPassive(
            MediaServer mediaServerItem, Map<String, Object> param, RequestCallback callback) {
    ...
    public JSONObject addStreamProxy(
            MediaServer mediaServerItem,
            String app,
            String stream,
            String url,
    ...
    public JSONObject connectRtpServer(
            MediaServer mediaServerItem, String dst_url, int dst_port, String stream_id) {
        Map<String, Object> param = new HashMap<>(1);
        param.put("dst_url", dst_url);
    ...
    public JSONObject updateRtpServerSSRC(
            MediaServer mediaServerItem, String streamId, String ssrc) {
        Map<String, Object> param = new HashMap<>(1);
        param.put("ssrc", ssrc);
        param.put("stream_id", streamId);
    ...
    public JSONObject deleteRecordDirectory(
