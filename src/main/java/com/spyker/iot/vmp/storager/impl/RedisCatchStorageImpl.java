...
import com.spyker.iot.vmp.gb28181.bean.AlarmChannelMessage;
import com.spyker.iot.vmp.gb28181.bean.Device;
import com.spyker.iot.vmp.gb28181.bean.ParentPlatformCatch;
...
import com.spyker.iot.vmp.storager.dao.DeviceChannelMapper;
import com.spyker.iot.vmp.storager.dao.DeviceMapper;
...
public class RedisCatchStorageImpl implements IRedisCatchStorage {
    ...
    public void updatePlatformCatchInfo(ParentPlatformCatch parentPlatformCatch) {
        String key =
                VideoManagerConstants.PLATFORM_CATCH_PREFIX
    ...
    public void delPlatformKeepalive(String platformGbId) {
        redisTemplate.delete(
                VideoManagerConstants.PLATFORM_KEEPALIVE_PREFIX
                        + userSetting.getServerId()
                        + "_"
                        + platformGbId);
    }
    ...
    public void delPlatformRegister(String platformGbId) {
        redisTemplate.delete(
                VideoManagerConstants.PLATFORM_REGISTER_PREFIX
                        + userSetting.getServerId()
                        + "_"
                        + platformGbId);
    }
    ...
    @Override
    public void updatePlatformRegisterInfo(
            String callId, PlatformRegisterInfo platformRegisterInfo) {
        String key =
                VideoManagerConstants.PLATFORM_REGISTER_INFO_PREFIX
                        + userSetting.getServerId()
                        + "_"
                        + callId;
    ...
    @Override
    public SendRtpItem querySendRTPServer(
    ...
    /** 删除RTP推送信息缓存 */
    @Override
    public void deleteSendRTPServer(
            String platformGbId, String channelId, String callId, String streamId) {
        if (streamId == null) {
            streamId = "*";
        }
        if (callId == null) {
    ...
    public void addStream(
        ...
        String key =
                VideoManagerConstants.WVP_SERVER_STREAM_PREFIX
                        + userSetting.getServerId()
                        + "_"
                        + type
                        + "_"
    ...
    public void updateDevice(Device device) {
        String key =
                VideoManagerConstants.DEVICE_PREFIX
                        + userSetting.getServerId()
                        + "_"
                        + device.getDeviceId();
    ...
    public void removeDevice(String deviceId) {
        String key =
                VideoManagerConstants.DEVICE_PREFIX + userSetting.getServerId() + "_" + deviceId;
        redisTemplate.delete(key);
    }
    ...
    public void updateGpsMsgInfo(GPSMsgInfo gpsMsgInfo) {
        String key =
                VideoManagerConstants.WVP_STREAM_GPS_MSG_PREFIX
                        + userSetting.getServerId()
                        + "_"
                        + gpsMsgInfo.getId();
    ...
    public List<GPSMsgInfo> getAllGpsMsgInfo() {
        String scanKey =
                VideoManagerConstants.WVP_STREAM_GPS_MSG_PREFIX + userSetting.getServerId() + "_*";
        List<GPSMsgInfo> result = new ArrayList<>();
        List<Object> keys = RedisUtil.scan(redisTemplate, scanKey);
        for (Object o : keys) {
            String key = (String) o;
            GPSMsgInfo gpsMsgInfo =
                    JsonUtil.redisJsonToObject(redisTemplate, key, GPSMsgInfo.class);
    ...
    @Override
    public void updateStreamAuthorityInfo(
            String app, String stream, StreamAuthorityInfo streamAuthorityInfo) {
        String key =
                VideoManagerConstants.MEDIA_STREAM_AUTHORITY
                        + userSetting.getServerId()
                        + "_"
                        + app
                        + "_"
    ...
    public List<StreamAuthorityInfo> getAllStreamAuthorityInfo() {
        String scanKey =
                VideoManagerConstants.MEDIA_STREAM_AUTHORITY + userSetting.getServerId() + "_*_*";
    ...
    public void addPushListItem(String app, String stream, MediaArrivalEvent event) {
        ...
        StreamPushItem streamPushItem =
                StreamPushItem.getInstance(event, userSetting.getServerId());
