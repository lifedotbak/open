package com.spyker.iot.vmp.storager.impl;
...
import com.spyker.iot.vmp.storager.dao.*;
...
/** 视频设备数据存储-jdbc实现 swwheihei 2020年5月6日 下午2:31:42 */
...
public class VideoManagerStorageImpl implements IVideoManagerStorage {
    ...
    @Autowired private DeviceMobilePositionMapper deviceMobilePositionMapper;
    ...
    @Autowired private PlatformCatalogMapper platformCatalogMapper;

    @Autowired private StreamProxyMapper streamProxyMapper;
    ...
    @Autowired private PlatformCatalogMapper catalogMapper;

    @Autowired private PlatformGbStreamMapper platformGbStreamMapper;
    ...
    public boolean resetChannels(String deviceId, List<DeviceChannel> deviceChannelList) {
        ...
        for (DeviceChannel deviceChannel : deviceChannelList) {
            ...
            if (allChannelMap.containsKey(deviceChannel.getChannelId())) {
                deviceChannel.setStreamId(
                        allChannelMap.get(deviceChannel.getChannelId()).getStreamId());
                deviceChannel.setHasAudio(
                        allChannelMap.get(deviceChannel.getChannelId()).isHasAudio());
                if (allChannelMap.get(deviceChannel.getChannelId()).isStatus()
                        != deviceChannel.isStatus()) {
                    List<String> strings =
                            platformChannelMapper.queryParentPlatformByChannelId(
                                    deviceChannel.getChannelId());
                    if (!CollectionUtils.isEmpty(strings)) {
                        strings.forEach(
                                platformId -> {
                                    eventPublisher.catalogEventPublish(
                                            platformId,
                                            deviceChannel,
                                            deviceChannel.isStatus()
                                                    ? CatalogEvent.ON
            ...
            if (!result && !addChannels.isEmpty()) {
                if (addChannels.size() > limitCount) {
                    for (int i = 0; i < addChannels.size(); i += limitCount) {
                        ...
                        result =
                                result
                                        || deviceChannelMapper.batchAdd(
                                                        addChannels.subList(i, toIndex))
                                                < 0;
                    }
                } else {
            ...
            if (!result && !updateChannels.isEmpty()) {
                if (updateChannels.size() > limitCount) {
                    for (int i = 0; i < updateChannels.size(); i += limitCount) {
                        ...
                        result =
                                result
                                        || deviceChannelMapper.batchUpdate(
                                                        updateChannels.subList(i, toIndex))
                                                < 0;
                    }
                } else {
            ...
            if (!result && !deleteChannels.isEmpty()) {
                ...
                if (deleteChannels.size() > limitCount) {
                    for (int i = 0; i < deleteChannels.size(); i += limitCount) {
                        ...
                        result =
                                result
                                        || deviceChannelMapper.batchDel(
                                                        deleteChannels.subList(i, toIndex))
                                                < 0;
                    }
                } else {
    ...
    @Override
    public List<DeviceChannelExtend> queryChannelsByDeviceIdWithStartAndLimit(
            String deviceId,
            ...
            int start,
            int limit) {
        return deviceChannelMapper.queryChannelsByDeviceIdWithStartAndLimit(
                deviceId, channelIds, null, query, hasSubChannel, online, start, limit);
    }

    @Override
    public List<DeviceChannel> queryChannelsByDeviceId(
    ...
    @Override
    public List<DeviceChannelExtend> queryChannelsByDeviceId(
            String deviceId, List<String> channelIds, Boolean online) {
        return deviceChannelMapper.queryChannelsWithDeviceInfo(
                deviceId, null, null, null, online, channelIds);
    }
     ...
     */
    @Override
    public synchronized List<MobilePosition> queryMobilePositions(
            String deviceId, String channelId, String startTime, String endTime) {
        return deviceMobilePositionMapper.queryPositionByDeviceIdAndTime(
    ...
    public boolean updateParentPlatform(ParentPlatform parentPlatform) {
        ...
        ParentPlatformCatch parentPlatformCatch =
                redisCatchStorage.queryPlatformCatchInfo(
                        parentPlatform.getServerGBId()); // .getDeviceGBId());
        if (parentPlatform.getId() == null) {
            ...
            if (parentPlatformCatch == null) { // serverGBId 已变化
                ParentPlatform parentPlatById =
                        platformMapper.getParentPlatById(parentPlatform.getId());
                // 使用旧的查出缓存ID
                parentPlatformCatch = new ParentPlatformCatch();
                parentPlatformCatch.setId(parentPlatform.getServerGBId());
    ...
    public DeviceChannel queryChannelInParentPlatform(String platformId, String channelId) {
        List<DeviceChannel> channels =
                platformChannelMapper.queryChannelInParentPlatform(platformId, channelId);
        if (channels.size() > 1) {
    ...
    @Override
    public List<PlatformCatalog> queryChannelInParentPlatformAndCatalog(
            String platformId, String catalogId) {
        List<PlatformCatalog> catalogs =
                platformChannelMapper.queryChannelInParentPlatformAndCatalog(platformId, catalogId);
        return catalogs;
    }

    @Override
    public List<PlatformCatalog> queryStreamInParentPlatformAndCatalog(
            String platformId, String catalogId) {
        List<PlatformCatalog> catalogs =
                platformGbStreamMapper.queryChannelInParentPlatformAndCatalogForCatalog(
                        platformId, catalogId);
        return catalogs;
    }
    ...
    public Device queryVideoDeviceByPlatformIdAndChannelId(String platformId, String channelId) {
        List<Device> devices =
                platformChannelMapper.queryVideoDeviceByPlatformIdAndChannelId(
    ...
    public Device queryDeviceInfoByPlatformIdAndChannelId(String platformId, String channelId) {
        List<Device> devices =
                platformChannelMapper.queryDeviceInfoByPlatformIdAndChannelId(
                        platformId, channelId);
        if (devices.size() > 1) {
            // 出现长度大于0的时候肯定是国标通道的ID重复了
            logger.warn("国标ID存在重复：{}", channelId);
    ...
    public List<DeviceChannel> queryGbStreamListInPlatform(String platformId) {
        return gbStreamMapper.queryGbStreamListInPlatform(
                platformId, userSetting.isUsePushingAsStatus());
    }
    ...
    @Override
    public List<StreamProxyItem> getStreamProxyListForEnableInMediaServer(
            String id, boolean enable) {
    ...
    public int addCatalog(PlatformCatalog platformCatalog) {
        ParentPlatform platform =
                platformMapper.getParentPlatByServerGBId(platformCatalog.getPlatformId());
        if (platform == null) {
            return 0;
        }
                ...
                switch (Integer.parseInt(platformCatalog.getId().substring(10, 13))) {
                    ...
                    case 215:
                        if (platformCatalog.getParentId().length() <= 8) {
                            ...
                            PlatformCatalog catalog =
                                    catalogMapper.selectByPlatFormAndCatalogId(
                                            platformCatalog.getPlatformId(),
                                            platformCatalog.getParentId());
                            if (catalog != null) {
                    ...
                    case 216:
                        if (platformCatalog.getParentId().length() <= 8) {
                            ...
                            if (catalog == null) {
                                logger.warn(
                                        "[添加目录] 无法获取目录{}的CivilCode和BusinessGroupId",
                                        platformCatalog.getPlatformId());
                                break;
                            }
                            platformCatalog.setCivilCode(catalog.getCivilCode());
                            if (Integer.parseInt(platformCatalog.getParentId().substring(10, 13))
                                    == 215) {
                                platformCatalog.setBusinessGroupId(platformCatalog.getParentId());
                            } else {
                                if (Integer.parseInt(
                                                platformCatalog.getParentId().substring(10, 13))
                                        == 216) {
                                    platformCatalog.setBusinessGroupId(
                                            catalog.getBusinessGroupId());
                                }
                            }
                        }
                        break;
        ...
        if (result > 0) {
            ...
            eventPublisher.catalogEventPublish(
                    platformCatalog.getPlatformId(), deviceChannel, CatalogEvent.ADD);
        }
        return result;
    }
    ...
    private int delCatalogExecute(String id, String platformId) {
        ...
        if (delresult > 0) {
            ...
            eventPublisher.catalogEventPublish(
                    platformId, deviceChannelForCatalog, CatalogEvent.DEL);
        }

        List<GbStream> gbStreams =
                platformGbStreamMapper.queryChannelInParentPlatformAndCatalog(platformId, id);
        if (gbStreams.size() > 0) {
            List<DeviceChannel> deviceChannelList = new ArrayList<>();
        ...
        List<PlatformCatalog> platformCatalogs =
                platformChannelMapper.queryChannelInParentPlatformAndCatalog(platformId, id);
        ...
        if (!allChildCatalog.isEmpty()) {
            ...
            if (allChildCatalog.size() > limitCount) {
                for (int i = 0; i < allChildCatalog.size(); i += limitCount) {
                    ...
                    delChannelresult +=
                            platformCatalogMapper.deleteAll(
                                    platformId, allChildCatalog.subList(i, toIndex));
                }
            } else {
    ...
    public int updateCatalog(PlatformCatalog platformCatalog) {
        ...
        if (result > 0) {
            DeviceChannel deviceChannel = getDeviceChannelByCatalog(platformCatalog);
            eventPublisher.catalogEventPublish(
                    platformCatalog.getPlatformId(), deviceChannel, CatalogEvent.UPDATE);
        }
        return result;
    }
    ...
    public int delRelation(PlatformCatalog platformCatalog) {
        if (platformCatalog.getType() == 1) {
            ...
            deviceChannel.setChannelId(platformCatalog.getId());
            eventPublisher.catalogEventPublish(
                    platformCatalog.getPlatformId(), deviceChannel, CatalogEvent.DEL);
            return platformChannelMapper.delByCatalogIdAndChannelIdAndPlatformId(platformCatalog);
        } else if (platformCatalog.getType() == 2) {
            List<GbStream> gbStreams =
                    platformGbStreamMapper.queryChannelInParentPlatformAndCatalog(
            ...
            for (GbStream gbStream : gbStreams) {
                if (gbStream.getGbId().equals(platformCatalog.getId())) {
                    ...
                    eventPublisher.catalogEventPublish(
                            platformCatalog.getPlatformId(), deviceChannel, CatalogEvent.DEL);
                    return platformGbStreamMapper.delByAppAndStream(
    ...
    @Override
    public List<ParentPlatform> queryPlatFormListForGBWithGBId(
            String channelId, List<String> platforms) {
        return platformChannelMapper.queryPlatFormListForGBWithGBId(channelId, platforms);
    }

    @Override
    public List<ParentPlatform> queryPlatFormListForStreamWithGBId(
