package com.spyker.iot.vmp.storager.dao;

import com.spyker.iot.vmp.gb28181.bean.Device;
import com.spyker.iot.vmp.gb28181.bean.DeviceChannel;
import com.spyker.iot.vmp.gb28181.bean.DeviceChannelInPlatform;
...
public interface DeviceChannelMapper {
    ...
    @Insert(
            "INSERT INTO wvp_device_channel (channel_id, device_id, name, manufacture, model, owner, civil_code, block, "
    ...
    List<DeviceChannelExtend> queryChannelsWithDeviceInfo(
            @Param("deviceId") String deviceId,
            @Param("parentChannelId") String parentChannelId,
            @Param("query") String query,
            @Param("hasSubChannel") Boolean hasSubChannel,
    ...
    @Select(
            "SELECT * FROM wvp_device_channel WHERE device_id=#{deviceId} AND channel_id=#{channelId}")
    DeviceChannel queryChannel(
    ...
    @Delete(
            "DELETE FROM wvp_device_channel WHERE device_id=#{deviceId} AND channel_id=#{channelId}")
    ...
    @Update(
            value = {
                "UPDATE wvp_device_channel SET status=false WHERE device_id=#{deviceId} AND channel_id=#{channelId}"
            })
    ...
    @Update(
            value = {
                "UPDATE wvp_device_channel SET status=true WHERE device_id=#{deviceId} AND channel_id=#{channelId}"
            })
    ...
    int cleanChannelsNotInList(
            @Param("deviceId") String deviceId, @Param("channels") List<DeviceChannel> channels);
    ...
    @Select(
            "select * from wvp_device_channel where longitude*latitude > 0 and device_id = #{deviceId}")
    ...
    List<DeviceChannel> getChannelsWithCivilCodeAndLength(
            @Param("deviceId") String deviceId,
    ...
    List<DeviceChannel> getChannelsByCivilCode(
            @Param("deviceId") String deviceId, @Param("parentId") String parentId);
    ...
    @Select(
            "select * from wvp_device_channel where device_id=#{deviceId} and civil_code not in "
    ...
    @Select(
            "select * from wvp_device_channel where device_id=#{deviceId} and SUBSTRING(channel_id, 11, 3)=#{typeCode}")
    List<DeviceChannel> getBusinessGroups(
            @Param("deviceId") String deviceId, @Param("typeCode") String typeCode);

    @Select(
            "select dc.id, dc.channel_id, dc.device_id, COALESCE(dc.custom_name, dc.name) AS name, dc.manufacture,dc.model,dc.owner, pc.civil_code,dc.block, "
    ...
    @Select(
            "select de.* from wvp_device de left join wvp_device_channel dc on de.device_id = dc.deviceId where dc.channel_id=#{channelId}")
    ...
    List<DeviceChannel> getSubChannelsByDeviceId(
            @Param("deviceId") String deviceId,
            @Param("parentId") String parentId,
            @Param("onlyCatalog") boolean onlyCatalog);
