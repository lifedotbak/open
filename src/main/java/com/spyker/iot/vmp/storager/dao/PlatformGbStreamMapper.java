...
import com.spyker.iot.vmp.gb28181.bean.PlatformGbStream;
...
public interface PlatformGbStreamMapper {
    ...
    List<ParentPlatform> selectByAppAndStream(
            @Param("app") String app, @Param("stream") String stream);
    ...
    @Select(
            "<script> "
                    ...
                    + "</script>")
    List<GbStream> queryChannelInParentPlatformAndCatalog(
            @Param("platformId") String platformId, @Param("catalogId") String catalogId);

    @Select(
            "select gs.gb_id as id, gs.name as name, pgs.platform_id as platform_id, pgs.catalog_id as catalog_id , 0 as children_count, 2 as type\n"
                    + "from wvp_gb_stream gs\n"
                    + "    left join wvp_platform_gb_stream pgs\n"
                    + "        on gs.gb_stream_id = pgs.gb_stream_id\n"
                    + "where pgs.platform_id=#{platformId} and pgs.catalog_id=#{catalogId}")
    List<PlatformCatalog> queryChannelInParentPlatformAndCatalogForCatalog(
            @Param("platformId") String platformId, @Param("catalogId") String catalogId);
    ...
    @Select(
            "<script> "
                    ...
                    + "</script> ")
    List<ParentPlatform> queryPlatFormListForGBWithGBId(
            @Param("app") String app,
            @Param("stream") String stream,
            @Param("platforms") List<String> platforms);

    @Delete(
            "DELETE from wvp_platform_gb_stream WHERE gb_stream_id = (select id from wvp_gb_stream where app=#{app} AND stream=#{stream}) AND platform_id=#{platformId}")
    ...
    @Delete(
            "<script> "
                    ...
                    + "</foreach>"
                    + "</script>")
    void delByAppAndStreamsByPlatformId(
            @Param("gbStreams") List<GbStream> gbStreams, @Param("platformId") String platformId);
