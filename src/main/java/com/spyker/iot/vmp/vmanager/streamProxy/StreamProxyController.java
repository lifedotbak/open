package com.spyker.iot.vmp.vmanager.streamProxy;
...
public class StreamProxyController {
    ...
    public DeferredResult<Object> save(@RequestBody StreamProxyItem param) {
        ...
        StreamProxyItem streamProxyItem =
                streamProxyService.getStreamProxyByAppAndStream(param.getApp(), param.getStream());
        ...
        streamProxyService.save(
                param,
                (code, msg, streamInfo) -> {
                    logger.info(
                            "[拉流代理] {}", code == ErrorCode.SUCCESS.getCode() ? "成功" : "失败： " + msg);
    ...
    public Map<String, String> getFFmpegCMDs(@RequestParam String mediaServerId) {
        ...
        if (mediaServerItem == null) {
            throw new ControllerException(
                    ErrorCode.ERROR100.getCode(), "流媒体： " + mediaServerId + "未找到");
        }
    ...
    public void del(@RequestParam String app, @RequestParam String stream) {
        ...
        if (app == null || stream == null) {
            throw new ControllerException(
                    ErrorCode.ERROR400.getCode(), app == null ? "app不能为null" : "stream不能为null");
        } else {
