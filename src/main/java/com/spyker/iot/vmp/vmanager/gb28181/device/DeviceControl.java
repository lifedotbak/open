 ...
 * @date 2021年2月1日
 */
package com.spyker.iot.vmp.vmanager.gb28181.device;
...
public class DeviceControl {
    ...
    public DeferredResult<ResponseEntity<String>> recordApi(
            @PathVariable String deviceId, @PathVariable String recordCmdStr, String channelId) {
        if (logger.isDebugEnabled()) {
        ...
        DeferredResult<ResponseEntity<String>> result =
                new DeferredResult<ResponseEntity<String>>(3 * 1000L);
        result.onTimeout(
                () -> {
        ...
        try {
            cmder.recordCmd(
                    ...
                    event -> {
                        ...
                        msg.setData(
                                String.format(
                                        "开始/停止录像操作失败，错误码： %s, %s", event.statusCode, event.msg));
                        resultHolder.invokeAllResult(msg);
                    },
                    null);
        } catch (InvalidArgumentException | SipException | ParseException e) {
    ...
    public DeferredResult<String> guardApi(
            @PathVariable String deviceId, @PathVariable String guardCmdStr) {
        if (logger.isDebugEnabled()) {
        ...
        try {
            cmder.guardCmd(
                    ...
                    event -> {
                        ...
                        msg.setData(
                                String.format(
                                        "布防/撤防操作失败，错误码： %s, %s", event.statusCode, event.msg));
    ...
    public DeferredResult<ResponseEntity<String>> resetAlarmApi(
        ...
        try {
            cmder.alarmCmd(
                    ...
                    event -> {
                        ...
                        msg.setKey(key);
                        msg.setData(
                                String.format("报警复位操作失败，错误码： %s, %s", event.statusCode, event.msg));
                        resultHolder.invokeResult(msg);
                    },
                    null);
        } catch (InvalidArgumentException | SipException | ParseException e) {
    ...
    public JSONObject iFrame(
            @PathVariable String deviceId, @RequestParam(required = false) String channelId) {
        if (logger.isDebugEnabled()) {
    ...
    public DeferredResult<String> homePositionApi(
            String deviceId,
            String channelId,
            Boolean enabled,
        ...
        String key =
                DeferredResultHolder.CALLBACK_CMD_DEVICECONTROL
                        + (ObjectUtils.isEmpty(channelId) ? deviceId : channelId);
        String uuid = UUID.randomUUID().toString();
        Device device = storager.queryVideoDevice(deviceId);
        try {
            cmder.homePositionCmd(
                    device,
                    channelId,
                    enabled,
                    resetTime,
                    ...
                    event -> {
                        ...
                        msg.setData(
                                String.format(
                                        "看守位控制操作失败，错误码： %s, %s", event.statusCode, event.msg));
                        resultHolder.invokeResult(msg);
                    },
                    null);
        } catch (InvalidArgumentException | SipException | ParseException e) {
    ...
    public void dragZoomIn(
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    String.format(
                            "设备拉框放大 API调用，deviceId：%s ，channelId：%s ，length：%d ，width：%d ，midpointx：%d ，midpointy：%d ，lengthx：%d ，lengthy：%d",
                            deviceId, channelId, length, width, midpointx, midpointy, lengthx,
                            lengthy));
        }
        Device device = storager.queryVideoDevice(deviceId);
    ...
    public void dragZoomOut(
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    String.format(
                            "设备拉框缩小 API调用，deviceId：%s ，channelId：%s ，length：%d ，width：%d ，midpointx：%d ，midpointy：%d ，lengthx：%d ，lengthy：%d",
                            deviceId, channelId, length, width, midpointx, midpointy, lengthx,
                            lengthy));
        }
        Device device = storager.queryVideoDevice(deviceId);
