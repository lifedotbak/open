package com.spyker.iot.vmp.vmanager.rtp;
...
public class RtpController {
    ...
    @Parameter(
            ...
            required = true)
    public OtherRtpSendInfo openRtpServer(
            Boolean isSend,
            @RequestParam(required = false) String ssrc,
            String callId,
            String stream,
            Integer tcpMode,
            String callBack) {
        ...
        logger.info(
                "[第三方服务对接->开启收流和获取发流信息] isSend->{}, ssrc->{}, callId->{}, stream->{}, tcpMode->{}, callBack->{}",
                isSend,
                ssrc,
                callId,
                stream,
        ...
        if (callBack != null) {
            Hook hook =
                    Hook.getInstance(
                            HookType.on_rtp_server_timeout, "rtp", stream, mediaServer.getId());
                    ...
                    (hookData) -> {
                        if (stream.equals(hookData.getStream())) {
                            ...
                            try {
                                ...
                                logger.error(
                                        "[第三方服务对接->开启收流和获取发流信息] 等待收流超时 callId->{}, 发送回调失败",
                                        callId,
                                        e);
                            }
        ...
        String key =
                VideoManagerConstants.WVP_OTHER_SEND_RTP_INFO
                        + userSetting.getServerId()
                        + "_"
                        + callId;
    ...
    public void sendRTP(
        ...
        logger.info(
                ...
                ptForAudio,
                ptForVideo);
        if (!((dstPortForAudio > 0 && !ObjectUtils.isEmpty(dstPortForAudio)
                || (dstPortForVideo > 0 && !ObjectUtils.isEmpty(dstIpForVideo))))) {
        ...
        if (!ObjectUtils.isEmpty(dstIpForAudio) && dstPortForAudio > 0) {
            sendRtpItemForAudio =
                    SendRtpItem.getInstance(
                            ...
                            dstPortForAudio,
                            !isUdp,
                            sendInfo.getSendLocalPortForAudio(),
                            ptForAudio);
        } else {
        ...
        if (!ObjectUtils.isEmpty(dstIpForVideo) && dstPortForVideo > 0) {
            sendRtpItemForVideo =
                    SendRtpItem.getInstance(
                            ...
                            dstPortForAudio,
                            !isUdp,
                            sendInfo.getSendLocalPortForVideo(),
                            ptForVideo);
        } else {
        ...
        if (streamReady) {
            if (sendRtpItemForVideo != null) {
                ...
                logger.info(
                        "[第三方服务对接->发送流] 视频流发流成功，callId->{}，param->{}", callId, sendRtpItemForVideo);
            ...
            if (sendRtpItemForAudio != null) {
                ...
                logger.info(
                        "[第三方服务对接->发送流] 音频流发流成功，callId->{}，param->{}", callId, sendRtpItemForAudio);
                redisTemplate.opsForValue().set(key, sendInfo);
            }
        } else {
            ...
            Hook hook =
                    Hook.getInstance(HookType.on_media_arrival, app, stream, mediaServer.getId());
            ...
            hookSubscribe.addSubscribe(
                    ...
                    (hookData) -> {
                        ...
                        if (sendRtpItemForVideo != null) {
                            mediaServerService.startSendRtp(mediaServer, null, sendRtpItemForVideo);
                            logger.info(
                                    "[第三方服务对接->发送流] 视频流发流成功，callId->{}，param->{}",
                                    callId,
                                    sendRtpItemForVideo);
                        ...
                        if (sendRtpItemForAudio != null) {
                            mediaServerService.startSendRtp(mediaServer, null, sendRtpItemForAudio);
                            logger.info(
                                    "[第三方服务对接->发送流] 音频流发流成功，callId->{}，param->{}",
                                    callId,
                                    sendRtpItemForAudio);
                            redisTemplate.opsForValue().set(key, finalSendInfo);
                        }
    ...
    public void closeSendRTP(String callId) {
        ...
        mediaServerService.stopSendRtp(
                mediaServerItem,
                sendInfo.getPushApp(),
                sendInfo.getPushStream(),
                sendInfo.getPushSSRC());
