package com.spyker.iot.vmp.vmanager.gb28181.playback;
...
public class PlaybackController {
    ...
    public DeferredResult<WVPResult<StreamContent>> start(
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    String.format("设备回放 API调用，deviceId：%s ，channelId：%s", deviceId, channelId));
        }

        String uuid = UUID.randomUUID().toString();
        String key = DeferredResultHolder.CALLBACK_CMD_PLAYBACK + deviceId + channelId;
        DeferredResult<WVPResult<StreamContent>> result =
                new DeferredResult<>(userSetting.getPlayTimeout().longValue());
    ...
    public void playStop(
        ...
        try {
            ...
            throw new ControllerException(
                    ErrorCode.ERROR100.getCode(), "发送bye失败： " + e.getMessage());
        }
    }
    ...
    public void playSpeed(@PathVariable String streamId, @PathVariable Double speed) {
        ...
        if (speed != 0.25 && speed != 0.5 && speed != 1 && speed != 2.0 && speed != 4.0) {
            ...
            throw new ControllerException(
                    ErrorCode.ERROR100.getCode(), "不支持的speed（0.25 0.5 1、2、4）");
        }
        Device device = storager.queryVideoDevice(inviteInfo.getDeviceId());
        try {
