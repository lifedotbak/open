package com.spyker.iot.vmp.vmanager.streamPush;
...
import com.spyker.iot.vmp.service.IMediaService;
import com.spyker.iot.vmp.service.IStreamPushService;
import com.spyker.iot.vmp.service.impl.StreamPushUploadFileHandler;
import com.spyker.iot.vmp.vmanager.bean.*;
...
public class StreamPushController {
    ...
    public DeferredResult<ResponseEntity<WVPResult<Object>>> uploadChannelFile(
        ...
        // 最多处理文件一个小时
        DeferredResult<ResponseEntity<WVPResult<Object>>> result =
        ...
        try {
            ...
            ExcelReader excelReader =
                    EasyExcel.read(
                                    inputStream,
                                    StreamPushExcelDto.class,
                                    new StreamPushUploadFileHandler(
                                            streamPushService,
                                            mediaServerService.getDefaultMediaServer().getId(),
                                            (errorStreams, errorGBs) -> {
                                                logger.info(
                                                        "通道导入成功，存在重复App+Stream为{}个，存在国标ID为{}个",
                                                        errorStreams.size(),
                                                        errorGBs.size());
                                                RequestMessage msg = new RequestMessage();
                                                msg.setKey(key);
                                                WVPResult<Map<String, List<String>>> wvpResult =
                                                        new WVPResult<>();
    ...
    public StreamContent getPlayUrl(
        ...
        StreamInfo streamInfo =
                mediaServerService.getStreamInfoByAppAndStreamWithCheck(
                        app, stream, mediaServerId, authority);
        if (streamInfo == null) {
