...
public class DeviceConfig {
        ...
        try {
            cmder.deviceBasicConfigCmd(
                    ...
                    event -> {
                        ...
                        msg.setKey(key);
                        msg.setData(
                                String.format("设备配置操作失败，错误码： %s, %s", event.statusCode, event.msg));
    ...
    public DeferredResult<String> configDownloadApi(
            @PathVariable String deviceId,
        ...
        String key =
                DeferredResultHolder.CALLBACK_CMD_CONFIGDOWNLOAD
                        + (ObjectUtils.isEmpty(channelId) ? deviceId : channelId);
        String uuid = UUID.randomUUID().toString();
        Device device = storager.queryVideoDevice(deviceId);
        try {
            cmder.deviceConfigQuery(
                    device,
                    channelId,
                    configType,
                    event -> {
                        ...
                        msg.setKey(key);
                        msg.setData(
                                String.format("获取设备配置失败，错误码： %s, %s", event.statusCode, event.msg));
                        resultHolder.invokeResult(msg);
                    });
        } catch (InvalidArgumentException | SipException | ParseException e) {
