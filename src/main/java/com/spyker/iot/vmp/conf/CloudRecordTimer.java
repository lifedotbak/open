...
public class CloudRecordTimer {
    ...
    private static final Logger logger = LoggerFactory.getLogger(CloudRecordTimer.class);
        ...
        for (MediaServer mediaServerItem : mediaServerItemList) {
            ...
            if (mediaServerItem.getRecordDay() > 0) {
                ...
                List<CloudRecordItem> cloudRecordItemList =
                        cloudRecordServiceMapper.queryRecordListForDelete(
                                lastDate, mediaServerItem.getId());
                ...
                for (CloudRecordItem cloudRecordItem : cloudRecordItemList) {
                    ...
                    boolean deleteResult =
                            mediaServerService.deleteRecordDirectory(
                                    mediaServerItem,
                                    cloudRecordItem.getApp(),
                                    cloudRecordItem.getStream(),
                                    date,
                                    cloudRecordItem.getFileName());
                    if (deleteResult) {
