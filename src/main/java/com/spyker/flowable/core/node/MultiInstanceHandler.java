    ...
    public boolean completionCondition(DelegateExecution execution) {
        ...
        for (DelegateExecution delegateExecution : executionList) {
            Boolean variableLocal =
                    delegateExecution.getVariableLocal(
                            ProcessInstanceConstant.VariableKey.APPROVE_RESULT, Boolean.class);
            ...
            if (okNum > 0) {
                entity.setVariable(
                        StrUtil.format(
                                "{}_{}",
                                node.getId(),
                                ProcessInstanceConstant.VariableKey.APPROVE_NODE_RESULT),
