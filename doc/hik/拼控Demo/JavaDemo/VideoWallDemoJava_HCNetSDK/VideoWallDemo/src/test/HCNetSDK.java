...
import test.HCNetSDK.NET_DVR_RECTCFG_EX;
import test.HCNetSDK.uStruAlarm;
...
public interface HCNetSDK extends StdCallLibrary {
    ...
    HCNetSDK INSTANCE = (HCNetSDK) Native.loadLibrary("hik_sdk_lib\\HCNetSDK",
            HCNetSDK.class);
    /***宏定义***/
    //解码器
    public static final int MATRIXDECODER_ABILITY_V41=0x260; //解码器能力集
    ...
    public static final int  MAX_VIDEOWALL_NUM = 16; 
    public static final int MAX_VW_DISPLAYPOSITION = 256; 
    public static final int MAX_VM_WIN_NUM   =  256; 
    public static final int MAX_WALL_WIN_COUNT =  256;
...
// ivms进入检测区域NET_IVMS_ENTER_REGION
    ...
    //电视墙显示画面分割模式
    public static final int MAX_DISPNUM_V41=32;
    public static final int MAX_SDI_RES=16;     //SDI显示通道最大支持分辨率数
    public static final int MAX_SUPPORT_RES=32;
    public static final int  MAX_WINDOWS_NUM=12;        //画面分割模式的数量  
...
**************************************************/
    ...
    public static final int  COMM_ALARM_SHIPSDETECTION = 0x4521;	 //浜鸿劯鎶撴媿缁撴灉淇℃伅
    ...
    public static class  NET_VCA_POLYGON  extends Structure
    {
    	...
    	public NET_VCA_POINT[]  struPos=new NET_VCA_POINT[VCA_MAX_POLYGON_POINT_NUM]; //多边形边界点,最多十个 
    }
    ...
    public static class NET_DVR_SHIPSINFO  extends Structure
    {
    	...
    	public NET_VCA_POLYGON  struShipsRect=new NET_VCA_POLYGON(); //船只区域，归一化值，相对于大图（可见光图、热成像图)的分辨率
    }
    //船只检测报警上传
    public static class NET_DVR_SHIPSDETECTION_ALARM  extends Structure
    {
    	public int   dwSize;
    	public NET_VCA_DEV_INFO  struDevInfo= new NET_VCA_DEV_INFO();   //设备信息
        ...
        public byte  byRes1;
        public NET_DVR_SHIPSINFO[] struShipInfo=new NET_DVR_SHIPSINFO[MAX_SHIPS_NUM/*20*/];//船只信息；最大支持20艘
...
public static class NET_DVR_DDNS_STREAM_CFG extends Structure
{
    ...
    public byte []  byRes1=new byte[3];
    public NET_DVR_IPADDR  struStreamServer;            //流媒体服务器地址
...
public static class NET_DVR_IPPARACFG_V40 extends Structure
{
    ...
    public NET_DVR_IPDEVINFO_V31  [] struIPDevInfo = new NET_DVR_IPDEVINFO_V31[MAX_IP_DEVICE_V40];      /* IP设备 */
...
public static class NET_DVR_IPALARMINCFG_V40 extends Structure
{
    ...
    public NET_DVR_IPALARMININFO_V40[] struIPAlarmInInfo = new NET_DVR_IPALARMININFO_V40[4096];/* IP报警输入*/
...
public static class NET_DVR_IPALARMOUTCFG_V40 extends Structure
{
    ...
    public NET_DVR_IPALARMOUTINFO_V40 []struIPAlarmOutInfo = new NET_DVR_IPALARMOUTINFO_V40[4096];/*IP报警输出*/
    public byte[]     byRes= new byte[256];
}
...
public static class STRU_DEVICE_INFO extends Structure{
	public int		iDeviceIndex;			//device index
	public NativeLong	lLoginID;				//ID
	public int	dwDevSoftVer;			//erserved
	
	public byte[]	chLocalNodeName= new byte[100];	//local device node
	public byte[]	chDeviceName= new byte[100];		//device name
	public byte[]	chDeviceIP= new byte[130];			//device IP: IP,pppoe address, or network gate address, etc
	public byte[]    chDeviceIPInFileName= new byte[130];  //if chDeviceIP includes ':'(IPV6),change it to '.', for '.'is not allowed in window's file name
	//public byte[]	chDevNetCard1IP[33];	//IP
	public byte[]	chLoginUserName= new byte[NAME_LEN];	//login user name
	public byte[]	chLoginPwd= new byte[NAME_LEN];	//password
	public byte[]	chDeviceMultiIP= new byte[130];	//multi-cast group address
	//char	chIPServerIP[32];		//IP
	public byte[]	chSerialNumber= new byte[50];		//SN
	...
	public NativeLong	lDevicePort;			//port number
	...
	public NativeLong  [] lAudioHandle= new NativeLong[MAX_AUDIO_V40]; //voicebroadcast handle
	public boolean	[]bCheckBroadcast= new boolean[MAX_AUDIO_V40]; //Add to broad cast group
	public NativeLong	lFortifyHandle;			//on guard handle
	...
	public int	dwImageType;			//device status icon
	public boolean	bIPRet;					//support IP conection
	public int		iEnableChanNum;			//valid channel number
	public boolean	bDVRLocalAllRec;		//local recording
	public NativeLong 	[]lVoiceCom = new NativeLong[MAX_AUDIO_V40];	//voice transmit
	public NativeLong 	lFirstEnableChanIndex;			//first enabled channel index
	public NativeLong 	lTranHandle;				//232 transparent channel handle
	...
	public NET_DVR_IPPARACFG_V40 pStruIPParaCfgV40 = new NET_DVR_IPPARACFG_V40();				//IP channel parameters
	public NET_DVR_IPALARMINCFG struAlarmInCfg = new NET_DVR_IPALARMINCFG();
	public NET_DVR_IPALARMINCFG_V40   pStruIPAlarmInCfgV40 = new NET_DVR_IPALARMINCFG_V40();  // IP alarm In parameters
	public NET_DVR_IPALARMOUTCFG_V40  pStruIPAlarmOutCfgV40 = new NET_DVR_IPALARMOUTCFG_V40(); // IP alarm Out parameters
	public NET_DVR_IPALARMOUTCFG struAlarmOutCfg = new NET_DVR_IPALARMOUTCFG();
	public STRU_DEVICE_INFO pNext = new STRU_DEVICE_INFO();
	public STRU_CHANNEL_INFO []struZeroChan = new STRU_CHANNEL_INFO[16];	
	public byte[] sSecretKey= new byte[16];
	public int iAudioEncType;
	public PASSIVEDECODE_CHANINFO []struPassiveDecode = new PASSIVEDECODE_CHANINFO[256];
	...
	public STRU_CHANNEL_INFO []struMirrorChan = new STRU_CHANNEL_INFO[16];
    ...
    public void Init()
    {
        ...
        iDeviceType = 0;
        iDiskNum = 0;
        lDevicePort = new NativeLong(8000);
        iAlarmInNum = 0;
        iAlarmOutNum = 0;
        iAnalogChanNum = 0;
        iIPChanNum = 0;
        ...
        bPlayDevice = false;
        bVoiceTalk = false;
        lAudioHandle = new NativeLong[MAX_AUDIO_V40];
        bCheckBroadcast = new boolean[MAX_AUDIO_V40];
        lFortifyHandle =new NativeLong(-1);
        bAlarm = false;
        iDeviceLoginType = 0;
        dwImageType = 1;
        bIPRet = false;
        pNext =null;
        lVoiceCom = new NativeLong[MAX_AUDIO_V40];
        for (int i = 0; i < lVoiceCom.length; i++)
        {
            lVoiceCom[i] = new NativeLong(-1);
        }
        lFirstEnableChanIndex = new NativeLong(0);
        lTranHandle = new NativeLong(-1);
        byZeroChanNum = 0;
        lAudioHandle[0] = new NativeLong(-1);
        lAudioHandle[1] = new NativeLong(-1);
        struAlarmInCfg = new NET_DVR_IPALARMINCFG();
        struAlarmOutCfg = new NET_DVR_IPALARMOUTCFG();
        byte[] by = "StreamNotEncrypt".getBytes();
...
public static class NET_DVR_VOD_PARA extends Structure
{
 ...
 public static class NET_DVR_FILECOND extends Structure //录象文件查找条件结构
	...
	public static class NET_DVR_SPECIAL_FINDINFO_UNION extends Structure
	{
		...
		public NET_DVR_ATMEINDINFO struATMFindInfo;
	}
...
public static class NET_DVR_MATRIX_ABILITY_V41 extends Structure{
	  ...
	  public byte [] byRes=new byte[5];                    
	  public NET_DVR_DISPINFO struVgaInfo=new NET_DVR_DISPINFO();
	  public NET_DVR_DISPINFO struBncInfo=new NET_DVR_DISPINFO();
	  public NET_DVR_DISPINFO struHdmiInfo=new NET_DVR_DISPINFO();
	  public NET_DVR_DISPINFO struDviInfo=new NET_DVR_DISPINFO();
	  ...
	  public byte[] byRes2=new byte[3];
	  public NET_DVR_SDI_INFO struSDIInfo;//SDI显示通道信息
	  public byte[] byRes3=new byte[48];

}

//解码器或者大屏显示通道信息结构体
public static class NET_DVR_DISPINFO extends Structure{
	public byte  byChanNums;//通道个数
	public byte  byStartChan;//起始通道
	public byte[] byRes=new byte[2];
    public int[]  dwSupportResolution=new int[MAX_SUPPORT_RES];//支持分辨率
}

//解码器显示通道画面分割模式结构体
public static class NET_DVR_DISPWINDOWMODE extends Structure{
	public byte  byDispChanType;//显示通道类型：0-BNC, 1-VGA, 2-HDMI, 3-DVI 4-SDI
	public byte  byDispChanSeq;//显示通道序号,从1开始，如果类型是VGA，则表示第几个VGA
	public byte[] byRes=new byte[2];
	public byte[] byDispMode=new byte[MAX_WINDOWS_NUM/*12*/];
}

//大屏拼接信息结构体
public static class NET_DVR_SCREENINFO extends Structure{
...
public static class NET_DVR_SDI_INFO extends Structure{
	public byte byChanNums;//通道个数
    public byte byStartChan;//起始通道
    public byte[] byRes=new byte[2];
    public int[] dwSupportResolution=new int[MAX_SDI_RES];//支持分辨率
}
...
//批量设置设备配置信息
public static class NET_DVR_SetDeviceConfigEx extends Structure{
	public NativeLong lUserID;
...
//电视墙显示模式
public static class NET_DVR_VIDEOWALLDISPLAYMODE extends Structure{
	public int dwSize;
	public byte byEnable;
	public byte[] byRes1=new byte[3];
...
public static class NET_DVR_DEC_DDNS_DEV extends Structure{
	  public NET_DVR_DEV_DDNS_INFO struDdnsInfo=new NET_DVR_DEV_DDNS_INFO();
	  public NET_DVR_STREAM_MEDIA_SERVER  struMediaServer=new NET_DVR_STREAM_MEDIA_SERVER();
}
...
public static class NET_DVR_VIDEOWALLDISPLAYPOSITION_EXTENDGET extends Structure{
	public int dwCount;
	public NET_DVR_VIDEOWALLDISPLAYPOSITION[] strDisPlayPos=(NET_DVR_VIDEOWALLDISPLAYPOSITION[]) new NET_DVR_VIDEOWALLDISPLAYPOSITION().toArray(256); ;
}

public static class NET_DVR_VIDEOWALLWINDOWPOSITION_EXTENDGET extends Structure{
	public int dwCount;
	public NET_DVR_VIDEOWALLWINDOWPOSITION[] strWinPos=(NET_DVR_VIDEOWALLWINDOWPOSITION[]) new NET_DVR_VIDEOWALLWINDOWPOSITION().toArray(256); ;
}

public static class NET_DVR_VIDEOWALLDISPLAYPOSITION_COND_4 extends Structure{
	public NET_DVR_VIDEOWALLDISPLAYPOSITION[] strDisPlayPos=(NET_DVR_VIDEOWALLDISPLAYPOSITION[]) new NET_DVR_VIDEOWALLDISPLAYPOSITION().toArray(4); ;
}

//输入参数结构体
public static class NET_DVR_IN_PARAM extends Structure{
	public NET_DVR_BUF_INFO struCondBuf=new NET_DVR_BUF_INFO();
	public NET_DVR_BUF_INFO struInParamBuf=new NET_DVR_BUF_INFO();
	public int  dwRecvTimeout;
	public byte[]   byRes=new byte[32];
}
//输出参数结构体
public static class NET_DVR_OUT_PARAM extends Structure{
	public NET_DVR_BUF_INFO struOutBuf=new NET_DVR_BUF_INFO();
...
public static class NET_DVR_WALLWIN_INFO extends Structure{
	public int dwSize;
	public int dwWinNum;   //窗口号
	public int dwSubWinNum;//子窗口号
	public int dwWallNo; //墙号
	public byte[]  byRes=new byte[12];
}
...
public static class NET_DVR_WALLWINPARAM extends Structure{
	...
	public byte  byRes1;
	public int dwAmplifyingSubWndNo; //全屏子窗口号（1字节墙号+1字节子窗口号+2字节窗口号）                                        //当byWndShowMode为2时有效，表示当前全屏显示的子窗口
    public byte  byWndTopKeep; //窗口置顶保持，0-不保持，1-保持
    public byte  byWndOpenKeep; //窗口打开保持，0-不保持，1-保持
    public byte[]  byRes=new byte[22];
}

public static class DwWallNo extends Structure{
	public int dwWallNo; //墙号
}

public static class WinNo extends Structure{
	public int winNo; //窗口号
}

public static class RetWinNo extends Structure{
	public int retWinNo; //窗口号
}

public static class Temp extends Structure{
	String[] pTemp=new String[4 + MAX_WALL_WIN_COUNT *(new NET_DVR_VIDEOWALLDISPLAYPOSITION()).size()];
}
public static class DisPlayWinNum extends Structure{
	public int[] disPlayWinNumber=new int[4];
}
...
public static class DwStatus extends Structure{
    public  int[] dwStatus =new int[MAX_DEVICES];
}
...
//NET_DVR_SetDVRMessage的扩展
 ...
 String   NET_DVR_GetErrorMsg(Pointer pErrorNo);
...
//云台控制相关接口
 ...
 boolean  NET_DVR_RemoteControl(NativeLong lUserID,int dwCommand,Pointer lpParam,int dwInBufferSize);
...
//参数配置 begin
 boolean  NET_DVR_GetDeviceAbility(NativeLong lUserID,int dwAbilityType,String pInBuf,int dwInLength,Pointer pOutBuf,int dwOutLength);
 ...
 boolean  NET_DVR_GetDeviceStatus(NativeLong lUserID, int dwCommand,int dwCount,Pointer lpInBuffer, int dwInBufferSize, Pointer lpStatusList, Pointer lpOutBuffer, int dwOutBufferSize);
 ...
 boolean  NET_DVR_SetDeviceConfig(NativeLong lUserID, int dwCommand, int dwCount, Pointer lpInBuffer, int dwInBufferSize, Pointer m_dwStatus, Pointer lpInParamBuffer, int dwInParamBufferSize);
 boolean  NET_DVR_SetDeviceConfigEx(NativeLong lUserID, int dwCommand, int dwCount, Pointer lpInParam,Pointer lpOutParam);
...
interface PlayCtrl extends StdCallLibrary
{
    ...
    public static final int SOFT_DECODE_ENGINE = 0;
    public static final int HARD_DECODE_ENGINE = 1;	
	boolean PlayM4_SetDecodeEngineEx(NativeLong nPort, int nDecodeEngine);
}
